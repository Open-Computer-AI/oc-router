  ...
  <AppLayout>
      ...
      <template #table>
        ...
        >
          <template #cell-key="{ value, row }">
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                class="rounded-lg p-1 transition-colors hover:bg-muted"
                :class="
                  copiedKeyId === row.id
                    ? 'text-green-500'
          ...
          <template #cell-group="{ row }">
            <div class="group/dropdown relative">
              ...
              >
                ...
                />
                <span v-else class="text-sm text-text-dim">{{
                  t('keys.noGroup')
                }}</span>
                <span class="text-xs text-text-muted">{{ t('keys.selectGroup') }}</span>
                <svg
                  class="h-3.5 w-3.5 text-text-dim opacity-60 transition-opacity group-hover/dropdown:opacity-100"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  stroke-width="2"
          ...
          <template #cell-usage="{ row }">
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                <span class="text-text-muted">{{ t('keys.today') }}:</span>
                <span class="font-medium text-text">
                  ${{ (usageStats[row.id]?.today_actual_cost ?? 0).toFixed(4) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-text-muted">{{ t('keys.total') }}:</span>
                <span class="font-medium text-text">
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  <span class="text-text-muted">{{ t('keys.quota') }}:</span>
                  <span :class="[
                    'font-medium',
            ...
            <div v-if="row.rate_limit_5h > 0 || row.rate_limit_1d > 0 || row.rate_limit_7d > 0" class="space-y-1.5 min-w-[140px]">
              ...
              <div v-if="row.rate_limit_5h > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-text-muted">5h</span>
                  <span :class="[
                ...
                <div v-if="row.reset_5h_at && formatResetTime(row.reset_5h_at)" class="text-[10px] text-text-dim tabular-nums">
              ...
              <div v-if="row.rate_limit_1d > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-text-muted">1d</span>
                  <span :class="[
                    'font-medium tabular-nums',
                ...
                <div v-if="row.reset_1d_at && formatResetTime(row.reset_1d_at)" class="text-[10px] text-text-dim tabular-nums">
              ...
              <div v-if="row.rate_limit_7d > 0">
                ...
                <div v-if="row.reset_7d_at && formatResetTime(row.reset_7d_at)" class="text-[10px] text-text-dim tabular-nums">
              ...
              <button
                ...
                class="mt-0.5 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-xs text-text-muted transition-colors hover:bg-muted hover:text-accent"
          ...
          <template #cell-expires_at="{ value }">
            <span v-if="value" :class="[
              'text-sm',
              new Date(value) < new Date() ? 'text-red-500 dark:text-red-400' : 'text-text-muted'
            ]">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-text-dim">{{ t('keys.noExpiration') }}</span>
          </template>
          ...
          <template #cell-last_used_at="{ value }">
            <span v-if="value" class="text-sm text-text-muted">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-text-dim">-</span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-text-muted transition-colors hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-900/20 dark:hover:text-green-400"
              >
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-text-muted transition-colors hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/20 dark:hover:text-blue-400"
              >
              ...
              <button
                ...
                :class="[
                  ...
                  row.status === 'active'
                    ? 'text-text-muted hover:bg-yellow-50 hover:text-yellow-600 dark:hover:bg-yellow-900/20 dark:hover:text-yellow-400'
                    : 'text-text-muted hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-900/20 dark:hover:text-green-400'
                ]"
              >
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-text-muted transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
              >
        ...
        <div>
          ...
          >
            <template #selected="{ option }">
              ...
              />
              <span v-else class="text-text-dim">{{ t('keys.selectGroup') }}</span>
            </template>
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                ...
                formData.use_custom_key ? 'bg-accent' : 'bg-muted'
              ]"
            >
              <span
                :class="[
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_ip_restriction ? 'bg-accent' : 'bg-muted'
              ]"
            >
              <span
                :class="[
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_quota ? 'bg-accent' : 'bg-muted'
              ]"
            >
              <span
                :class="[
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_rate_limit ? 'bg-accent' : 'bg-muted'
              ]"
            >
              <span
                :class="[
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_expiration ? 'bg-accent' : 'bg-muted'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.enable_expiration" class="space-y-4 pt-2">
            ...
            <div v-if="showEditModal && selectedKey?.expires_at" class="text-sm">
              <span class="text-text-muted">{{ t('keys.currentExpiration') }}: </span>
              <span class="font-medium text-text">
          ...
          {{ t('keys.ccsClientSelect.description') }}
	        ...
	        <div class="grid grid-cols-2 gap-3">
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-border hover:border-accent hover:bg-accent-muted transition-all"
	          >
	            <Icon name="terminal" size="xl" class="text-text-muted" />
	            <span class="font-medium text-text">{{
	          ...
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-border hover:border-accent hover:bg-accent-muted transition-all"
	          >
	            <Icon name="sparkles" size="xl" class="text-text-muted" />
	            <span class="font-medium text-text">{{
    ...
    <Teleport to="body">
      <div
        ...
        ref="dropdownRef"
        class="animate-in fade-in slide-in-from-top-2 fixed z-[100000020] w-max min-w-[380px] overflow-hidden rounded-xl bg-bg-elevated shadow-lg ring-1 ring-black/5 dark:ring-white/10 duration-200"
      ...
      >
        ...
        <div class="border-b border-border p-2">
          <div class="relative">
            <svg class="absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-text-dim" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <input
              v-model="groupSearchQuery"
              type="text"
              class="w-full rounded-lg border border-border bg-muted py-1.5 pl-8 pr-3 text-sm text-text placeholder-text-dim outline-none focus:border-accent focus:ring-1 focus:ring-accent"
        ...
        <div class="max-h-80 overflow-y-auto p-1.5">
          ...
          <div v-if="filteredGroupOptions.length === 0" class="py-4 text-center text-sm text-text-dim">
            {{ t('keys.noGroupFound') }}
          </div>
        </div>
      </div>
