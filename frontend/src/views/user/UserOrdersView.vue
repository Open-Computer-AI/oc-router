<template>
  <AppLayout>
    ...
    <BaseDialog :show="!!cancelTargetId" :title="t('payment.orders.cancel')" width="narrow" @close="cancelTargetId = null">
      <p class="text-sm text-text-muted">{{ t('payment.confirmCancel') }}</p>
      <template #footer>
      ...
      <div v-if="refundTarget" class="space-y-4">
        <div class="rounded-xl bg-bg p-4">
          <div class="flex justify-between text-sm">
            ...
            <span class="font-mono text-text">#{{ refundTarget.id }}</span>
          </div>
          <div class="mt-2 flex justify-between text-sm">
            <span class="text-text-muted">{{ t('payment.orders.amount') }}</span>
            <span class="text-text">${{ refundTarget.amount.toFixed(2) }}</span>
          </div>
        </div>
        <div>
