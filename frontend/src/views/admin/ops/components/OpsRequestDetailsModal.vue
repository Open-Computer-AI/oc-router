      ...
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-if="loading" class="flex flex-1 items-center justify-center py-16">
          <div class="flex flex-col items-center gap-3">
            ...
            <span class="text-sm font-medium text-text-muted">{{ t('common.loading') }}</span>
          </div>
        </div>
        ...
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-xl border border-dashed border-border p-10 text-center">
            <div class="text-sm font-medium text-text">{{ t('admin.ops.requestDetails.empty') }}</div>
            <div class="mt-1 text-xs text-text-dim">{{ t('admin.ops.requestDetails.emptyHint') }}</div>
          </div>
          ...
          <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border">
            <div class="min-h-0 flex-1 overflow-auto">
              ...
              <tbody class="divide-y divide-border bg-bg-elevated">
                <tr v-for="(row, idx) in items" :key="idx" class="hover:bg-muted">
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-xs font-medium text-text">
                    {{ (row.platform || 'unknown').toUpperCase() }}
                  </td>
                  <td class="max-w-[240px] truncate px-4 py-3 text-xs text-text" :title="row.model || ''">
                    {{ row.model || '-' }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 text-xs text-text">
                  ...
                  <td class="px-4 py-3">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      <span class="max-w-[220px] truncate font-mono text-[11px] text-text" :title="row.request_id">
                        {{ row.request_id }}
                      </span>
                      <button
                        class="rounded-md bg-muted px-2 py-1 text-[10px] font-bold text-text-muted hover:bg-muted"
