...
const healthScoreClass = computed(() => {
  if (isSystemIdle.value) return 'text-text-dim'
  const score = healthScoreValue.value
...
<template>
  <div :class="['flex flex-col gap-4 rounded-3xl bg-bg-elevated shadow-sm ring-1 ring-border', props.fullscreen ? 'p-8' : 'p-6']">
    <!-- Top Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-4">
      <div>
        <h1 class="flex items-center gap-2 font-display text-xl font-black text-text">
        ...
        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-text-muted">
          <span class="flex items-center gap-1.5" :title="props.loading ? t('admin.ops.loadingText') : t('admin.ops.ready')">
            <span class="relative flex h-2 w-2">
              <span class="relative inline-flex h-2 w-2 rounded-full" :class="props.loading ? 'bg-text-dim' : 'bg-green-500'"></span>
            </span>
          ...
          <template v-if="props.autoRefreshEnabled && props.autoRefreshCountdown !== undefined">
            <span>·</span>
            <span>Next in {{ props.autoRefreshCountdown }}s</span>
          </template>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-3">
        <template v-if="!props.fullscreen">
          ...
          <div class="mx-1 hidden h-4 w-[1px] bg-muted sm:block"></div>
        ...
        <button
          ...
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-muted text-text-muted transition-colors hover:bg-muted"
          :disabled="loading"
        ...
        <div v-if="!props.fullscreen" class="mx-1 hidden h-4 w-[1px] bg-muted sm:block"></div>
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-muted px-3 text-xs font-bold text-text transition-colors hover:bg-muted"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-muted text-text transition-colors hover:bg-muted"
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div :class="['rounded-2xl bg-muted lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          <!-- 1) Health Score -->
          <div
            class="group relative flex cursor-pointer flex-col items-center justify-center rounded-xl py-2 transition-all hover:bg-muted md:border-r md:border-border md:pr-6"
          >
            ...
            >
              <div class="rounded-xl bg-bg-elevated p-4 shadow-xl ring-1 ring-black/5">
                <h4 class="mb-3 border-b border-border pb-2 text-sm font-bold text-text flex items-center gap-2">
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    ...
                    <div class="flex-1">
                      <div class="text-xs font-semibold text-text">{{ item.message }}</div>
                      <div class="mt-0.5 text-[11px] text-text-muted">{{ item.impact }}</div>
                ...
                <div class="mt-3 border-t border-border pt-2 text-[10px] text-text-dim">
            ...
            <div class="relative flex items-center justify-center">
              ...
              <div class="absolute flex flex-col items-center">
                ...
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase tracking-wider text-text-dim']">{{ t('admin.ops.health') }}</span>
              </div>
            </div>

            <div class="mt-4 text-center" v-if="!props.fullscreen">
              <div class="flex items-center justify-center gap-1 text-xs font-medium text-text-muted">
          ...
          <div class="flex h-full flex-col justify-center py-2">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                ...
                <h3 class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.realtime.title') }}</h3>
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-text-dim']">{{ t('admin.ops.current') }}</div>
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-text']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-text-muted']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-text']">{{ displayRealTimeTps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-text-muted']">{{ t('admin.ops.tps') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-text-dim']">{{ t('admin.ops.peak') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-text-muted']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-text">{{ realtimeQpsPeakLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-text">{{ realtimeTpsPeakLabel }}</span>
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-text-dim']">{{ t('admin.ops.average') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-text-muted']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-text">{{ realtimeQpsAvgLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-text">{{ realtimeTpsAvgLabel }}</span>
                      <span class="text-xs">{{ t('admin.ops.tps') }}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        <!-- Card 1: Requests -->
        <div class="rounded-2xl bg-muted p-4" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-text-dim">{{ t('admin.ops.requestsTitle') }}</span>
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              <span class="text-text-muted">{{ t('admin.ops.requests') }}:</span>
              <span class="font-bold text-text">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-text-muted">{{ t('admin.ops.tokens') }}:</span>
              <span class="font-bold text-text">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-text-muted">{{ t('admin.ops.avgQps') }}:</span>
              <span class="font-bold text-text">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-text-muted">{{ t('admin.ops.avgTps') }}:</span>
              <span class="font-bold text-text">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="rounded-2xl bg-muted p-4" style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-bold uppercase text-text-dim">{{ t('admin.ops.sla') }}</span>
          ...
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              <span class="text-text-muted">{{ t('admin.ops.exceptions') }}:</span>
              <span class="font-bold text-danger">{{ formatNumber((overview.request_count_sla ?? 0) - (overview.success_count ?? 0)) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 4: Request Duration -->
        <div class="rounded-2xl bg-muted p-4" style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-text-dim">{{ t('admin.ops.latencyDuration') }}</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            ...
            <span class="text-xs font-bold text-text-dim">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-text-muted">P95:</span>
              <span class="font-bold text-text">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-text-dim">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-text-muted">P90:</span>
              <span class="font-bold text-text">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-text-dim">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-text-muted">P50:</span>
              <span class="font-bold text-text">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-text-dim">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-text-muted">Avg:</span>
              <span class="font-bold text-text">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-text-dim">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-text-muted">Max:</span>
              <span class="font-bold text-text">{{ durationMaxMs ?? '-' }}</span>
              <span class="text-text-dim">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="rounded-2xl bg-muted p-4" style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-text-dim">TTFT</span>
        ...
        <div class="rounded-2xl bg-muted p-4" style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-text-dim">{{ t('admin.ops.requestErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-text-muted">{{ t('admin.ops.errorCount') }}:</span>
              <span class="font-bold text-text">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-text-muted">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="font-bold text-text">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 6: Upstream Errors -->
        <div class="rounded-2xl bg-muted p-4" style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-text-dim">{{ t('admin.ops.upstreamErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-text-muted">{{ t('admin.ops.errorCountExcl429529') }}:</span>
              <span class="font-bold text-text">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-text-muted">429/529:</span>
              <span class="font-bold text-text">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="mt-2 border-t border-border pt-4">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <div class="rounded-xl bg-muted p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-text-dim">CPU</div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-text-muted">
        ...
        <div class="rounded-xl bg-muted p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.memory') }}</div>
        ...
        <div class="rounded-xl bg-muted p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.db') }}</div>
        ...
        <div class="rounded-xl bg-muted p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-text-dim">Redis</div>
        ...
        <div class="rounded-xl bg-muted p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.goroutines') }}</div>
        ...
        <div class="rounded-xl bg-muted p-3">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              <div class="text-[10px] font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.jobs') }}</div>
    ...
    <BaseDialog :show="showJobsDetails" :title="t('admin.ops.jobs')" width="wide" @close="showJobsDetails = false">
      <div v-if="!jobHeartbeats.length" class="text-sm text-text-muted">
        {{ t('admin.ops.noData') }}
      </div>
      <div v-else class="space-y-3">
        ...
        >
          <div class="flex items-center justify-between gap-3">
            <div class="truncate text-sm font-semibold text-text">{{ hb.job_name }}</div>
          ...
          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-text-muted sm:grid-cols-2">
            <div>
      ...
      <div class="space-y-4">
        <div>
          ...
          <input
            ...
            type="datetime-local"
            class="w-full rounded-lg border border-border bg-bg-elevated px-3 py-2 text-sm text-text focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-text mb-1">
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button
            type="button"
            class="rounded-lg bg-muted px-4 py-2 text-sm font-medium text-text hover:bg-muted"
