  ...
  <BaseDialog :show="show" :title="title" width="full" :close-on-click-outside="true" @close="close">
    <div v-if="loading" class="flex items-center justify-center py-16">
      <div class="flex flex-col items-center gap-3">
        ...
        <div class="text-sm font-medium text-text-muted">{{ t('admin.ops.errorDetail.loading') }}</div>
      </div>
    </div>

    <div v-else-if="!detail" class="py-10 text-center text-sm text-text-muted">
      {{ emptyText }}
    </div>
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.requestId') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-text">
            {{ requestId || '—' }}
          </div>
        </div>

        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.time') }}</div>
        ...
        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">
        ...
        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.platform') }}</div>
          <div class="mt-1 text-sm font-medium text-text">
            {{ detail.platform || '—' }}
          </div>
        </div>
        ...
        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.group') }}</div>
          <div class="mt-1 text-sm font-medium text-text">
        ...
        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.model') }}</div>
          <div class="mt-1 text-sm font-medium text-text">
            <template v-if="hasModelMapping(detail)">
              <span class="font-mono">{{ detail.requested_model }}</span>
              <span class="mx-1 text-text-dim">→</span>
              <span class="font-mono text-accent">{{ detail.upstream_model }}</span>
            </template>
            <template v-else>
              {{ displayModel(detail) || '—' }}
            </template>
        ...
        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.inboundEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-text">
        ...
        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.upstreamEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-text">
        ...
        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.status') }}</div>
          <div class="mt-1">
        ...
        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.requestType') }}</div>
          <div class="mt-1 text-sm font-medium text-text">
        ...
        <div class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.message') }}</div>
          <div class="mt-1 truncate text-sm font-medium text-text" :title="detail.message">
            {{ detail.message || '—' }}
          </div>
        </div>

        <div v-if="detail.api_key_prefix" class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.apiKeyPrefix') }}</div>
          <div class="mt-1 font-mono text-sm font-medium text-text">
            {{ detail.api_key_prefix }}
          </div>
        </div>

        <div v-if="detail.attempted_key_prefix" class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.attemptedKeyPrefix') }}</div>
          <div class="mt-1 font-mono text-sm font-medium text-text">
            {{ detail.attempted_key_prefix }}
          </div>
        </div>

        <div v-if="detail.deleted_key_owner_email" class="rounded-xl bg-muted p-4">
          <div class="text-xs font-bold uppercase tracking-wider text-text-dim">{{ t('admin.ops.errorDetail.deletedKeyOwner') }}</div>
          <div class="mt-1 text-sm font-medium text-text">
            {{ detail.deleted_key_owner_email }}
            <span v-if="detail.deleted_key_name" class="ml-1 text-xs text-text-muted">({{ detail.deleted_key_name }})</span>
      ...
      <div class="rounded-xl bg-muted p-6">
        <h3 class="text-sm font-black uppercase tracking-wider text-text">{{ t('admin.ops.errorDetail.responseBody') }}</h3>
        <pre class="mt-4 max-h-[520px] overflow-auto rounded-xl border border-border bg-bg-elevated p-4 text-xs text-text"><code>{{ prettyJSON(primaryResponseBody || '') }}</code></pre>
      </div>

      <!-- Upstream errors list (only for request errors) -->
      <div v-if="showUpstreamList" class="rounded-xl bg-muted p-6">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h3 class="text-sm font-black uppercase tracking-wider text-text">{{ t('admin.ops.errorDetails.upstreamErrors') }}</h3>
          <div class="text-xs text-text-muted" v-if="correlatedUpstreamLoading">{{ t('common.loading') }}</div>
        </div>

        <div v-if="!correlatedUpstreamLoading && !correlatedUpstreamErrors.length" class="mt-3 text-sm text-text-muted">
          {{ t('common.noData') }}
        </div>
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-text">
                #{{ idx + 1 }}
                <span v-if="ev.type" class="ml-2 rounded-md bg-muted px-2 py-0.5 font-mono text-[10px] font-bold text-text">{{ ev.type }}</span>
              </div>
              <div class="flex items-center gap-2">
                ...
                <button
                  type="button"
                  class="inline-flex items-center gap-1.5 rounded-md px-1.5 py-1 text-[10px] font-bold text-accent hover:bg-accent-muted disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <div class="mt-3 grid grid-cols-1 gap-2 text-xs text-text sm:grid-cols-2">
              <div>
                <span class="text-text-dim">{{ t('admin.ops.errorDetail.upstreamEvent.status') }}:</span>
                <span class="ml-1 font-mono">{{ ev.status_code ?? '—' }}</span>
              </div>
              <div>
                <span class="text-text-dim">{{ t('admin.ops.errorDetail.upstreamEvent.requestId') }}:</span>
                <span class="ml-1 font-mono">{{ ev.request_id || ev.client_request_id || '—' }}</span>
              </div>
            </div>

            <div v-if="ev.message" class="mt-3 break-words text-sm font-medium text-text">{{ ev.message }}</div>

            <pre
              v-if="expandedUpstreamDetailIds.has(ev.id)"
              class="mt-3 max-h-[240px] overflow-auto rounded-xl border border-border bg-muted p-3 text-xs text-text"
...
const statusClass = computed(() => {
  ...
  return 'bg-muted text-text ring-border'
})
