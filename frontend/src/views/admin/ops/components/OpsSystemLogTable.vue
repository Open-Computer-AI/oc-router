...
const resetRuntimeConfig = async () => {
  const ok = window.confirm('Reset to startup config (env/yaml) and apply immediately?')
  if (!ok) return
  ...
  try {
    ...
    runtimeConfig.stacktrace_level = saved.stacktrace_level
    runtimeConfig.retention_days = saved.retention_days
    appStore.showSuccess('Reset to startup log config')
    await fetchHealth()
  } catch (err: any) {
    console.error('[OpsSystemLogTable] Failed to reset runtime log config', err)
    appStore.showError(err?.response?.data?.detail || 'Failed to reset log config')
  } finally {
    runtimeSaving.value = false
  }
}
...
const cleanupCurrentFilter = async () => {
  const ok = window.confirm('Clean up system logs matching the current filter? This action cannot be undone.')
  if (!ok) return
  try {
    const payload = {
...
<template>
  <section class="rounded-2xl border border-border bg-bg-elevated p-4 shadow-sm">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-text">System Logs</h3>
        <p class="mt-1 text-xs text-text-muted">Sorted by most recent. Supports filtering, searching, and cleanup by condition.</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-muted px-2 py-1 text-text">Queue {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-muted px-2 py-1 text-text">Written {{ health.written_count }}</span>
    ...
    <div class="mb-4 rounded-xl border border-border bg-muted p-3">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-text">Runtime Log Config (takes effect immediately)</div>
        <span v-if="runtimeLoading" class="text-xs text-text-muted">Loading...</span>
      </div>
      ...
      <p v-if="health.last_error" class="mt-2 text-xs text-danger">Recent write error: {{ health.last_error }}</p>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border border-border">
      <div v-if="loading" class="px-4 py-8 text-center text-sm text-text-muted">Loading...</div>
      <div v-else-if="!hasData" class="px-4 py-8 text-center text-sm text-text-muted">No system logs found</div>
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-border">
          <thead class="bg-muted">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-text-muted">Time</th>
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-text-muted">Level</th>
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-text-muted">Log Details</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-border">
            <tr v-for="row in logs" :key="row.id" class="align-top">
              <td class="px-3 py-2 text-xs text-text">{{ formatTime(row.created_at) }}</td>
              <td class="px-3 py-2 text-xs">
                <span class="inline-flex rounded-full px-2 py-0.5 font-semibold" :class="levelBadgeClass(row.level)">
                  {{ row.level }}
                </span>
              </td>
              <td class="px-3 py-2 text-xs text-text whitespace-normal break-all">
