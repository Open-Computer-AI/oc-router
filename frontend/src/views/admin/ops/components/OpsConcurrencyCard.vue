...
// Refresh节奏由父组件统一控制（OpsDashboard Header 的RefreshStatus/倒计时）
watch(
  () => props.refreshToken,
  () => {
...
<template>
  <div class="flex h-full flex-col rounded-3xl bg-bg-elevated p-6 shadow-sm ring-1 ring-border">
    <!-- 头部 -->
    <div class="mb-4 flex shrink-0 items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <!-- Refresh按钮 -->
        <button
          class="flex items-center gap-1 rounded-lg bg-muted px-2 py-1 text-[11px] font-semibold text-text transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50"
          :disabled="loading"
    ...
    <div
      ...
      class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-border text-sm text-text-muted"
    >
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border">
      ...
      <!-- 空Status -->
      <div v-if="displayRows.length === 0" class="flex flex-1 items-center justify-center text-sm text-text-muted">
      ...
      <div v-else-if="displayDimension === 'user'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as UserRow[])" :key="row.key" class="rounded-lg bg-muted p-2.5">
          <!-- User信息和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span class="truncate text-[11px] font-bold text-text" :title="row.username || row.user_email">
                {{ row.username || row.user_email }}
              </span>
              <span v-if="row.username" class="shrink-0 truncate text-[10px] text-text-dim" :title="row.user_email">
                {{ row.user_email }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-text"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
      ...
      <div v-else-if="displayDimension === 'platform' || displayDimension === 'group'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as SummaryRow[])" :key="row.key" class="rounded-lg bg-muted p-3">
          <!-- 标题行 -->
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              <div class="truncate text-[11px] font-bold text-text" :title="row.name">
                {{ row.name }}
              </div>
              <span v-if="displayDimension === 'group' && row.platform" class="text-[10px] text-text-dim">
                {{ row.platform.toUpperCase() }}
              </span>
            </div>
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-bold text-text"> {{ row.used_concurrency }}/{{ row.total_concurrency }} </span>
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <div class="flex items-center gap-1">
              <svg class="h-3 w-3 text-text-dim" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path
              ...
              <span class="text-text-dim">{{ row.availability_percentage }}%</span>
            </div>
      ...
      <div v-else class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as AccountRow[])" :key="row.key" class="rounded-lg bg-muted p-2.5">
          <!-- 账号名称和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-0.5 text-[9px] text-text-dim">
                {{ row.group_name }}
              </div>
            </div>
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span class="font-mono text-[11px] font-bold text-text"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
              <!-- Status徽章 -->
              <span
              ...
              <span
                v-else
                class="inline-flex items-center gap-1 rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium text-text-muted"
              >
