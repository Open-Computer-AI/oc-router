<template>
  <div class="flex h-full min-h-0 flex-col bg-bg-elevated">
    <!-- Loading State -->
      ...
      <div class="min-h-0 flex-1 overflow-auto border-b border-border">
        <table class="w-full border-separate border-spacing-0">
          <thead class="sticky top-0 z-10 bg-muted">
            <tr>
              <th class="border-b border-border px-4 py-2.5 text-left text-[11px] font-bold uppercase tracking-wider text-text-muted">
              ...
              <th class="border-b border-border px-4 py-2.5 text-right text-[11px] font-bold uppercase tracking-wider text-text-muted">
          ...
          <tbody class="divide-y divide-border">
            <tr v-if="rows.length === 0">
              <td colspan="12" class="py-12 text-center text-sm text-text-dim">
            ...
            <tr
              v-for="log in rows"
              :key="log.id"
              class="group cursor-pointer transition-colors hover:bg-muted"
            ...
            >
              ...
              <td class="px-4 py-2">
                <div class="max-w-[160px]">
                  <el-tooltip v-if="log.inbound_endpoint" :content="formatEndpointTooltip(log)" placement="top" :show-after="500">
                    <span class="truncate font-mono text-[11px] text-text">
              ...
              <!-- Platform -->
              <td class="whitespace-nowrap px-4 py-2">
                <span class="inline-flex items-center rounded bg-muted px-1.5 py-0.5 text-[10px] font-bold uppercase text-text-muted">
                  {{ log.platform || '-' }}
                </span>
              </td>
                ...
                <div class="max-w-[160px]">
                  <template v-if="hasModelMapping(log)">
                    <el-tooltip :content="modelMappingTooltip(log)" placement="top" :show-after="500">
                      <span class="flex items-center gap-1 truncate font-mono text-[11px] text-text">
                        <span class="truncate">{{ log.requested_model }}</span>
                        <span class="flex-shrink-0 text-text-dim">→</span>
                        <span class="truncate text-accent">{{ log.upstream_model }}</span>
                      </span>
                    </el-tooltip>
                  </template>
                  <template v-else>
                    <span v-if="displayModel(log)" class="truncate font-mono text-[11px] text-text" :title="displayModel(log)">
                      {{ displayModel(log) }}
                    </span>
                    <span v-else class="text-xs text-text-dim">-</span>
                  </template>
              ...
              <td class="px-4 py-2">
                 <el-tooltip v-if="log.group_id" :content="t('admin.ops.errorLog.id') + ' ' + log.group_id" placement="top" :show-after="500">
                  <span class="max-w-[100px] truncate text-xs font-medium text-text">
                    {{ log.group_name || '-' }}
                  </span>
                </el-tooltip>
                <span v-else class="text-xs text-text-dim">-</span>
              ...
              <td class="px-4 py-2">
                <el-tooltip v-if="log.user_id" :content="t('admin.ops.errorLog.userId') + ' ' + log.user_id" placement="top" :show-after="500">
                  <span class="block max-w-[140px] truncate text-xs font-medium text-text">
                    {{ log.user_email || '-' }}
                  </span>
                </el-tooltip>
                <span v-else class="text-xs text-text-dim">-</span>
              ...
              <td class="px-4 py-2">
                <div v-if="log.api_key_id || log.api_key_name" class="flex max-w-[140px] items-center gap-1">
                  <span class="truncate text-xs font-medium text-text" :title="log.api_key_name || ('#' + log.api_key_id)">
              ...
              <td class="px-4 py-2">
                <el-tooltip v-if="log.account_id" :content="t('admin.ops.errorLog.accountId') + ' ' + log.account_id" placement="top" :show-after="500">
                  <span class="block max-w-[120px] truncate text-xs font-medium text-text">
                    {{ log.account_name || '-' }}
                  </span>
                </el-tooltip>
                <span v-else class="text-xs text-text-dim">-</span>
              ...
              <td class="whitespace-nowrap px-4 py-2">
                <div class="flex items-center gap-1.5">
                  ...
                  <span
                    ...
                    class="rounded bg-muted px-1.5 py-0.5 text-[10px] font-bold text-text-muted"
                  >
              ...
              <td class="px-4 py-2">
                <div class="max-w-[200px]">
                  <p class="truncate text-[11px] font-medium text-text-muted" :title="log.message">
              ...
              <td class="whitespace-nowrap px-4 py-2 text-right" @click.stop>
                <div class="flex items-center justify-end gap-3">
                  <button type="button" class="text-accent hover:text-accent text-xs font-bold" @click="emit('openErrorDetail', log.id)">
...
function getTypeBadge(log: OpsErrorLog): { label: string; className: string } {
  ...
  if (phase === 'internal' && owner === 'platform') {
    return { label: t('admin.ops.errorLog.typeInternal'), className: 'bg-muted text-text ring-border' }
  }

    const fallback = phase || owner || t('common.unknown')
    return { label: fallback, className: 'bg-muted text-text ring-border' }
}
...
function getStatusClass(code: number): string {
  ...
  return 'bg-muted text-text-muted ring-border'
}
