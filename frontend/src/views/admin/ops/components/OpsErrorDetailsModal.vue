  ...
  <BaseDialog :show="show" :title="modalTitle" width="full" @close="close">
    <div class="flex h-full min-h-0 flex-col">
      ...
      <div class="mb-4 flex-shrink-0 border-b border-border pb-4">
        <div class="grid grid-cols-8 gap-2">
          <div class="col-span-2 compact-select">
            <div class="relative group">
              <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                <svg
                  class="h-3.5 w-3.5 text-text-dim transition-colors group-focus-within:text-accent"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <input
                v-model="q"
                type="text"
                class="w-full rounded-lg border-border bg-muted py-1.5 pl-9 pr-3 text-xs font-medium text-text transition-all focus:border-accent focus:bg-bg-elevated focus:ring-2 focus:ring-accent/10"
          ...
          <div class="flex items-center justify-end">
            <button type="button" class="rounded-lg bg-muted px-3 py-1.5 text-xs font-semibold text-text transition-colors hover:bg-muted" @click="resetFilters">
              {{ t('common.reset') }}
            </button>
          </div>
        </div>
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <div class="mb-2 flex-shrink-0 text-xs text-text-muted">
