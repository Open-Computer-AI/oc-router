...
<template>
  <div class="rounded-3xl bg-bg-elevated p-6 shadow-sm ring-1 ring-border">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-text">{{ t('admin.ops.alertRules.title') }}</h3>
        <p class="mt-1 text-xs text-text-muted">{{ t('admin.ops.alertRules.description') }}</p>
      </div>
    ...
    <div v-else-if="sortedRules.length === 0" class="rounded-xl border border-dashed border-border p-8 text-center text-sm text-text-muted">
      {{ t('admin.ops.alertRules.empty') }}
    </div>

    <div v-else class="max-h-[520px] overflow-hidden rounded-xl border border-border">
      <div class="max-h-[520px] overflow-y-auto">
        <table class="min-w-full divide-y divide-border">
          ...
          <tbody class="divide-y divide-border bg-bg-elevated">
            <tr v-for="row in sortedRules" :key="row.id" class="hover:bg-muted">
              <td class="px-4 py-3">
                <div class="text-xs font-bold text-text">{{ row.name }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-text-muted">
                  {{ row.description }}
                </div>
                <div v-if="row.updated_at" class="mt-1 text-[10px] text-text-dim">
                  {{ formatDateTime(row.updated_at) }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-text">
                <span class="font-mono">{{ row.metric_type }}</span>
                <span class="mx-1 text-text-dim">{{ row.operator }}</span>
                <span class="font-mono">{{ row.threshold }}</span>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs font-bold text-text">
                {{ row.severity }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-text">
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div v-if="selectedMetricDefinition" class="mt-1 space-y-0.5 text-xs text-text-muted">
          ...
          <div class="flex items-center justify-between rounded-xl bg-muted px-4 py-3 md:col-span-2">
            <span class="text-xs font-bold text-text">{{ t('admin.ops.alertRules.form.enabled') }}</span>
            <input v-model="draft!.enabled" type="checkbox" class="h-4 w-4 rounded border-border text-accent focus:ring-accent" />
          </div>

          <div class="flex items-center justify-between rounded-xl bg-muted px-4 py-3 md:col-span-2">
            <span class="text-xs font-bold text-text">{{ t('admin.ops.alertRules.form.notifyEmail') }}</span>
            <input v-model="draft!.notify_email" type="checkbox" class="h-4 w-4 rounded border-border text-accent focus:ring-accent" />
          </div>
        </div>
      </div>
