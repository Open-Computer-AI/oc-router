      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'security'" class="space-y-6">
          ...
          <div class="card">
            ...
            >
              <h2 class="text-lg font-display font-semibold text-text">
              ...
              <div v-else class="space-y-4">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <code
                      class="rounded bg-muted px-2 py-1 font-mono text-sm text-text"
                    >
                      {{ adminApiKeyMasked }}
                    </code>
                  </div>
                ...
                >
                  ...
                  <div class="flex items-center gap-2">
                    <code
                      class="flex-1 select-all break-all rounded border border-green-300 bg-bg-elevated px-3 py-2 font-mono text-sm dark:border-green-700"
                    >
                      {{ newAdminApiKey }}
                    </code>
                    <button
              ...
              <template v-else>
                ...
                >
                  <div class="mb-3 flex items-center gap-2">
                    ...
                    <span
                      class="rounded bg-muted px-2 py-0.5 text-xs text-text-muted"
                    >
                      {{ rule.beta_token }}
                    </span>
                  </div>
                  ...
                  <div v-if="betaPresets[rule.beta_token]?.length" class="mt-3">
                    ...
                    <div class="flex flex-wrap gap-2">
                      <button
                        ...
                        :key="preset.label"
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md border border-accent/20 bg-accent-muted px-2.5 py-1 text-xs font-medium text-accent transition-colors hover:bg-accent-muted/80"
                  ...
                  <div class="mt-3">
                    ...
                    <div class="flex flex-wrap items-center gap-1.5">
                      ...
                      <button
                        ...
                        :key="pattern"
                        type="button"
                        class="rounded border border-border px-2 py-0.5 text-xs text-text-muted transition-colors hover:border-accent hover:bg-accent-muted hover:text-accent"
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <div
                ...
                class="rounded-lg border border-dashed border-border p-6 text-center text-sm text-text-muted"
              >
        ...
        <div v-show="activeTab === 'security'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <div class="border-t border-border pt-4">
                ...
                <div
                  class="mt-3 rounded-lg border border-border bg-bg-elevated p-2"
                >
                  <div class="flex flex-wrap items-center gap-2">
                    <span
                      v-for="suffix in registrationEmailSuffixWhitelistTags"
                      :key="suffix"
                      class="inline-flex items-center gap-1 rounded bg-muted px-2 py-1 text-xs font-mono text-text"
                    >
                      <span>{{ suffix }}</span>
                      <button
                        type="button"
                        class="rounded-full text-text-muted hover:bg-muted hover:text-text"
                        @click="
                    ...
                    >
                      <input
                        ...
                        type="text"
                        class="w-full bg-transparent text-sm font-mono text-text outline-none placeholder:text-text-dim"
                        :placeholder="
                          t(
                  ...
                  <div>
                    ...
                    >
                      ...
                      <code
                        ...
                        class="select-all break-all rounded bg-muted px-2 py-1 font-mono text-xs text-text-muted"
                      >
          ...
          <!-- GitHub / Google Email Quick Login -->
          <div class="card">
            <div
              class="border-b border-border px-6 py-4"
            >
              <h2 class="text-lg font-display font-semibold text-text">
                {{ localText("Email Quick Login", "Email OAuth Sign-in") }}
              </h2>
              <p class="mt-1 text-sm text-text-muted">
                {{
                  localText(
                    "After enabling GitHub or Google email OAuth login, the system reads verified email - logs in directly if it exists, or auto-registers if not.",
              ...
              <div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
                <div class="rounded-lg border border-border p-4">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p class="mt-1 text-sm text-text-muted">
                        {{
                          localText(
                            "GitHub OAuth App requires read:user user:email permissions, fill in the backend callback URL below.",
                  ...
                  <div v-if="form.github_oauth_enabled" class="mt-4 space-y-4">
                    <div class="rounded-lg bg-muted px-3 py-2 text-xs text-text-muted">
                      <template v-if="isZhLocale">
                        ...
                        >OAuth Apps</a>
                        -> New OAuth App; Homepage URL is your site domain, Authorization callback URL is the backend callback URL below.
                      </template>
                      <template v-else>
                        Setup guide: GitHub Settings → Developer settings →
                        <a
                    ...
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                      <div>
                        <label class="mb-2 block text-sm font-medium text-text">Client ID</label>
                        <input
                      ...
                      <div>
                        <label class="mb-2 block text-sm font-medium text-text">Client Secret</label>
                        <input
                          ...
                          :placeholder="
                            form.github_oauth_client_secret_configured
                              ? localText('Secret configured. Leave empty to keep the current value.', 'Secret configured. Leave empty to keep the current value.')
                    ...
                    <div>
                      <label class="mb-2 block text-sm font-medium text-text">
                        {{ localText("Backend Callback URL", "Backend Callback URL") }}
                      </label>
                      <input
                      ...
                      <div class="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
                        ...
                        >
                          {{ localText("Generate and copy", "Generate and copy") }}
                        </button>
                        <code
                    ...
                    <div>
                      <label class="mb-2 block text-sm font-medium text-text">
                        {{ localText("Frontend Callback URL", "Frontend Callback URL") }}
                      </label>
                      <input
                ...
                <div class="rounded-lg border border-border p-4">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p class="mt-1 text-sm text-text-muted">
                        {{
                          localText(
                            "Google OAuth client requires openid email profile scope, and register the backend callback URL in credentials.",
                  ...
                  <div v-if="form.google_oauth_enabled" class="mt-4 space-y-4">
                    <div class="rounded-lg bg-muted px-3 py-2 text-xs text-text-muted">
                      {{
                        localText(
                          "Setup guide: Google Cloud Console -> APIs & Services -> OAuth consent screen to complete consent screen; Credentials -> Create Credentials -> OAuth client ID, select Web application type, and add the URL below to Authorized redirect URIs.",
            ...
            <div class="space-y-5 p-6">
              ...
              >
                <div class="space-y-4">
                  ...
                  >
                    <div class="flex items-start justify-between gap-4">
                      <div>
                        <h3 class="font-medium text-text">
                          {{ localText("PC App", "PC App") }}
                        </h3>
                        <p class="mt-1 text-sm text-text-muted">
                          {{
                            localText(
                              "Desktop browser login via WeChat Open Platform QR code scan. Can coexist with Official Account or mobile app.",
                    ...
                    >
                      <div>
                        ...
                        <input
                          ...
                          :placeholder="
                            localText(
                              'WeChat Open Platform PC App AppID',
                        ...
                        <input
                          ...
                          :placeholder="
                            form.wechat_connect_open_app_secret_configured
                              ...
                              : localText(
                                  'WeChat Open Platform PC App AppSecret',
                  ...
                  >
                    <div class="flex items-start justify-between gap-4">
                      <div>
                        <h3 class="font-medium text-text">
                          {{ localText("Official Account", "Official Account") }}
                        </h3>
                        <p class="mt-1 text-sm text-text-muted">
                          {{
                            localText(
                              "Available only in WeChat in-app browser; shows unavailable in non-WeChat environments.",
                    ...
                    >
                      <div>
                        <label
                          class="mb-2 block text-sm font-medium text-text"
                        >
                          {{ localText("Official Account AppID", "Official Account App ID") }}
                        </label>
                        <input
                ...
                >
                  {{
                    localText(
                      "如果同时Enable PC App和Official Account/移动应用，这些应用需要挂在同一个微信开放平台主体下，No则 UnionID 无法稳定归并账号。",
                ...
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                  <div>
                    ...
                    <p class="mt-1.5 text-xs text-text-muted">
                      {{
                        localText(
                          "用于 PC App和Official Account的网页回调。移动应用走原生 SDK 时不直接使用这个浏览器回调。",
                ...
                <div class="grid grid-cols-1 gap-6">
                  ...
                  >
                    ...
                    <div v-if="form.dingtalk_connect_sync_display_name" class="space-y-2">
                      <div class="flex items-center gap-2">
                        <label class="text-sm text-text-muted whitespace-nowrap min-w-[5rem]">
                    ...
                    <p v-if="form.dingtalk_connect_sync_display_name" class="text-xs text-text-dim">
                    ...
                    <div v-if="form.dingtalk_connect_sync_corp_email" class="space-y-2">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <input
                          ...
                          type="text"
                          placeholder="钉钉企业Email"
                          class="input text-sm flex-1 max-w-xs"
                        />
                      </div>
                    </div>
                    <p v-if="form.dingtalk_connect_sync_corp_email" class="text-xs text-text-dim">
                  ...
                  >
                    ...
                    <p v-if="form.dingtalk_connect_sync_dept" class="text-xs text-text-dim">
            ...
            <div class="space-y-6 p-6">
              ...
              <div class="border-t border-border pt-4">
                ...
                <div
                  ...
                  class="rounded border border-dashed border-border px-4 py-3 text-sm text-text-muted"
                >
        ...
        <div v-show="activeTab === 'gateway'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <!-- YesNo允许在 Claude Code 中使用 Codex 插件（全局开关） -->
              <div class="flex items-center justify-between">
                <div class="pr-4">
            ...
            <div class="space-y-5 p-6">
              ...
              <div v-if="webSearchConfig.enabled" class="space-y-4">
                ...
                <div
                  ...
                  class="rounded-lg border border-dashed border-border p-4 text-center text-sm text-text-dim"
                >
                ...
                >
                  ...
                  >
                    <div class="flex items-center gap-3">
                      <svg
                        class="h-4 w-4 text-text-dim transition-transform"
                    ...
                    <div>
                      ...
                      <div class="relative">
                        ...
                        >
                          <button
                            type="button"
                            class="rounded p-1 text-text-dim hover:text-text-muted"
                            :title="
                              apiKeyVisible[pIdx]
                                ? t(
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <div
                        v-if="
                          ...
                          provider.quota_limit > 0
                        "
                        class="flex-1 rounded-full bg-muted"
                        style="height: 6px"
                      >
                        <div
          ...
          >
            <div
              class="mx-4 w-full max-w-lg rounded-xl bg-bg-elevated p-6 shadow-xl"
            >
              <h3
                class="mb-4 text-lg font-display font-semibold text-text"
              >
        ...
        <div v-show="activeTab === 'general'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="space-y-6 p-6">
              ...
              <div>
                ...
                <button
                  type="button"
                  class="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border px-4 py-2.5 text-sm text-text-muted transition-colors hover:border-accent hover:text-accent"
                  @click="addEndpoint"
                >
                  <svg
                    class="h-4 w-4"
          ...
          <div class="card">
            ...
            <div class="space-y-4 p-6">
                ...
                <div class="mb-3 flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <button
                      v-if="index > 0"
                      type="button"
                      class="rounded p-1 text-text-dim hover:bg-muted hover:text-text-muted"
              ...
              <button
                type="button"
                class="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border py-3 text-sm text-text-muted transition-colors hover:border-accent hover:text-accent"
                @click="addMenuItem"
              >
                <svg
                  class="h-4 w-4"
	          ...
	          <div class="card">
	            <div class="border-b border-border px-6 py-4">
	              <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
	                <div>
	                  <h2 class="text-lg font-display font-semibold text-text">
	                    {{ localText("Login条款Confirm", "Login agreement") }}
	                  </h2>
	                  <p class="mt-1 text-sm text-text-muted">
	                    {{
	                      localText(
	                        "控制Login页YesNo要求User先阅读并同意服务条款、隐私政策或其他 Markdown 文档。",
	                ...
	                <div class="flex items-center gap-3">
	                  <span class="text-sm text-text-muted">
	                    {{ form.login_agreement_enabled ? localText("Enabled", "Enabled") : localText("未Enable", "Disabled") }}
	                  </span>
	            ...
	            <div class="space-y-6 p-6">
                  ...
                  <p class="mt-1.5 text-xs text-text-muted">
                    {{
                      form.login_agreement_mode === "checkbox"
                        ? localText("复选框会显示在Login按钮下方，未勾选前所有Login入口Disable。", "The checkbox appears below the login button and gates all login actions.")
                        : localText("弹窗会在Login页打开，User拒绝后所有Login入口保持Disable。", "The modal opens on the login page and gates all login actions until accepted.")
                    }}
                  </p>
                </div>
                ...
                <div>
                  ...
                  />
                  <p class="mt-1.5 text-xs text-text-muted">
                    {{ localText("日期或文档内容变化后，User需要重新同意。", "Changing the date or content requires fresh consent.") }}
                  </p>
                </div>
              </div>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p class="mt-1 text-xs text-text-muted">
                      {{
                        localText(
                          "文档名称可自定义，内容按 Markdown Save。可参考：服务条款、使用政策、支持的国家和地区、服务特定条款。",
                  ...
                  >
                    <div class="mb-3 flex items-center justify-between gap-3">
                      <div class="flex min-w-0 items-center gap-3">
                        <span class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-md bg-muted text-text">
                          <Icon
                            :name="
                              index === 1
                                ? 'shield'
                    ...
                    <div class="grid grid-cols-1 gap-3 lg:grid-cols-2">
                      ...
                      <div>
                        ...
                        <div class="flex overflow-hidden rounded-lg border border-border bg-bg-elevated focus-within:border-accent focus-within:ring-1 focus-within:ring-accent">
                          <span class="inline-flex flex-shrink-0 items-center border-r border-border bg-muted px-3 text-sm text-text-muted">
                            /legal/
                          </span>
                          <input
                            v-model="doc.id"
                            type="text"
                            class="min-w-0 flex-1 border-0 bg-transparent px-3 py-2 text-sm text-text outline-none placeholder:text-text-dim focus:ring-0"
              ...
              <div class="border-t border-border pt-6">
                  ...
                  <table class="min-w-full divide-y divide-border">
                    <thead class="bg-muted">
                      <tr>
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-text-muted">{{ t('admin.settings.features.affiliate.customUsers.col.email') }}</th>
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-text-muted">{{ t('admin.settings.features.affiliate.customUsers.col.username') }}</th>
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-text-muted">{{ t('admin.settings.features.affiliate.customUsers.col.code') }}</th>
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-text-muted">{{ t('admin.settings.features.affiliate.customUsers.col.rate') }}</th>
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-text-muted">{{ t('admin.settings.features.affiliate.customUsers.col.actions') }}</th>
                      </tr>
                    </thead>
                    <tbody class="divide-y divide-border bg-bg-elevated">
                      ...
                      <tr v-for="entry in affiliateState.entries" :key="entry.user_id">
                        ...
                        <td class="px-3 py-2 text-sm text-text">{{ entry.email }}</td>
                        <td class="px-3 py-2 text-sm text-text-muted">{{ entry.username }}</td>
                        <td class="px-3 py-2 text-sm font-mono">
                          {{ entry.aff_code }}
                          <span
                            v-if="entry.aff_code_custom"
                            class="ml-1 inline-block rounded bg-accent-muted px-1.5 py-0.5 text-[10px] font-medium text-accent"
                        ...
                        <td class="px-3 py-2 text-sm">
                          ...
                          <span v-else class="text-text-dim">{{ t('admin.settings.features.affiliate.customUsers.useGlobal') }}</span>
                        </td>
                        <td class="px-3 py-2 text-sm">
                          <div class="flex items-center gap-2">
                ...
                <div v-if="affiliateState.total > affiliateState.pageSize" class="mt-3 flex items-center justify-between text-sm">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-text-muted">{{ affiliateState.page }} / {{ Math.max(1, Math.ceil(affiliateState.total / affiliateState.pageSize)) }}</span>
                    <button
                      type="button"
                      class="btn btn-secondary btn-sm"
        ...
        >
          <div class="w-full max-w-md rounded-lg bg-bg-elevated p-6 shadow-xl">
            ...
            <div class="space-y-4">
              <div v-if="affiliateModal.mode === 'add'">
                ...
                <div
                  ...
                  class="flex items-center justify-between rounded-md border border-accent/20 bg-accent-muted px-3 py-2"
                >
                  <div class="text-sm">
                    <span class="font-medium text-text">{{ affiliateModal.selectedUser.email }}</span>
                    <span class="ml-1 text-xs text-text-muted">({{ affiliateModal.selectedUser.username }})</span>
                  </div>
                  <button
                    type="button"
                    class="text-lg leading-none text-text-dim hover:text-red-600"
                ...
                <template v-else>
                  ...
                  >
                    ...
                    >
                      {{ u.email }} <span class="text-xs text-text-muted">({{ u.username }})</span>
                    </button>
                  </div>
                </template>
              </div>
              ...
              <template v-if="form.payment_enabled">
                ...
                <div class="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <div
                      class="rounded-lg border border-border bg-muted px-3 py-2 text-sm text-text-muted"
                    >
                      {{
                        (form.payment_product_name_prefix || "OC Router") +
                        " 100 " +
                ...
                <div class="grid grid-cols-2 gap-3 sm:grid-cols-5">
                  ...
                  <div>
                    ...
                    <div class="relative">
                      ...
                      />
                      <span
                        class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3 text-text-dim"
                        >%</span
                      >
                    </div>
                    <p class="mt-0.5 text-xs text-text-dim">
                    ...
                    <button
                      ...
                      :class="[
                        ...
                        isPaymentTypeEnabled(pt.value)
                          ...
                          : 'border-border bg-bg-elevated text-text-muted hover:border-border hover:bg-muted',
                      ]"
                    >
                      {{ pt.label }}
                    </button>
            ...
            <div class="p-6">
              <div class="flex items-start gap-3">
                <Icon
                  name="mail"
                  size="md"
                  class="mt-0.5 flex-shrink-0 text-text-dim"
                />
                <div>
                  <h3 class="font-medium text-text">
          ...
          <div class="card">
            ...
            <div class="px-6 py-6">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <label
                    class="mb-0 block text-sm font-medium text-text"
                  >
...
const paymentGuideHref = computed(() =>
  locale.value.startsWith("zh")
    ? "https://github.com/opencomputer/oc-router/blob/main/docs/PAYMENT_CN.md"
    : "https://github.com/opencomputer/oc-router/blob/main/docs/PAYMENT.md",
);

const paymentMethodsHref = computed(() =>
  locale.value.startsWith("zh")
    ? "https://github.com/opencomputer/oc-router/blob/main/docs/PAYMENT_CN.md#支持的支付方式"
    : "https://github.com/opencomputer/oc-router/blob/main/docs/PAYMENT.md#supported-payment-methods",
);
...
// Rate Limit Cooldown (429) Status
...
// OpenAI Fast/Flex Policy Status
const openaiFastPolicyForm = reactive({
  rules: [] as OpenAIFastPolicyRule[],
});
// 标记 openai_fast_policy_settings YesNo已成功从后端加载，
// 避免后端 GET 出错或字段缺失时，Save把默认规则覆盖成空数组。
...
const form = reactive<SettingsForm>({
  ...
  dingtalk_connect_sync_corp_email_attr_name: "钉钉企业Email",
...
const authSourceDefaultsMeta = computed(() => [
  ...
  },
  {
    ...
    description: localText(
      "通过 GitHub 已Verify Email首次Register或首次绑定时应用。",
  ...
  },
  {
    ...
    title: "Google",
    description: localText(
      "通过 Google 已Verify Email首次Register或首次绑定时应用。",
...
async function loadSettings() {
  ...
  try {
    ...
    // 仅当 payload 真的包含该字段时填充并标记为已加载；No则保持表单空值，
...
async function saveSettings() {
  ...
  try {
    ...
    if (form.login_agreement_enabled && normalizedLoginAgreementDocuments.length === 0) {
      appStore.showError(
        localText(
          "EnableLogin条款Confirm时，至少需要保留一份文档。",
    ...
    if (duplicateLoginAgreementDocumentId) {
      appStore.showError(
        localText(
          `Login条款文档路由不能重复：/legal/${duplicateLoginAgreementDocumentId}`,
    ...
    if (form.wechat_connect_mp_enabled && form.wechat_connect_mobile_enabled) {
      appStore.showError(
        localText(
          "Official Account和移动应用不能同时Enable。",
...
// =========================
// Affiliate (邀请返利) 专属User Management
// =========================
...
// 立即把相关字段Reset为 false，避免Save请求里残留旧值。后端 admin handler 与
// 配置加载层都有 coerce 兜底，这里Yes UX 层的同步而非安全防线。
watch(
  () => form.dingtalk_connect_corp_restriction_policy,
  (policy) => {
...
.settings-tabs-shell {
  @apply sticky z-20 -mx-1 rounded-2xl border border-border bg-bg-elevated/95 p-1.5 backdrop-blur-xl;
  top: 0.5rem;
}
...
.settings-tab {
  @apply relative isolate flex h-9 shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border border-transparent px-3 text-[13px] font-medium text-text-muted outline-none transition-colors duration-150;
}
...
.settings-tab:focus-visible {
  @apply ring-2 ring-accent/40 ring-offset-2 ring-offset-bg;
}

.settings-tab-active {
  @apply border-accent/15 bg-bg-elevated text-accent shadow-sm;
}
...
.settings-tab-icon {
  @apply flex h-5 w-5 shrink-0 items-center justify-center rounded text-text-muted transition-colors duration-150;
}
