  ...
  <AppLayout>
            ...
            <template #after>
              ...
              <div class="relative" ref="autoRefreshDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-56 origin-top-right rounded-lg border border-border bg-bg-elevated shadow-lg"
                >
                  <div class="p-2">
                    <button
                      @click="setAutoRefreshEnabled(!autoRefreshEnabled)"
                      class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-text hover:bg-muted"
                    >
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-[min(20rem,calc(100vw-2rem))] origin-top-right overflow-hidden rounded-lg border border-border bg-bg-elevated shadow-xl"
                >
                  <div class="max-h-[70vh] overflow-y-auto p-2">
                    <div class="px-2 py-2">
                      <div class="text-xs font-semibold uppercase tracking-wide text-text-dim">
                    ...
                    <button class="account-tools-menu-item" @click="openExportDataDialogFromMenu">
                      ...
                      <span
                        v-if="selIds.length"
                        class="rounded-full bg-accent-muted px-2 py-0.5 text-xs font-medium text-accent"
                      >
                    ...
                    <div class="px-2 py-2">
                      <div class="flex items-center justify-between gap-3">
                        <span class="text-xs font-semibold uppercase tracking-wide text-text-dim">
                          {{ t('admin.accounts.viewColumns') }}
                        </span>
                        <Icon name="grid" size="sm" class="text-text-dim" />
                      </div>
                    </div>
                    <div class="grid grid-cols-1 gap-1">
                      <button
                        ...
                        class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-text transition-colors hover:bg-muted"
                      >
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-select="{ row }">
            <input type="checkbox" :checked="isSelected(row.id)" @change="toggleSel(row.id)" class="rounded border-border text-accent focus:ring-accent" />
          </template>
          ...
          <template #cell-notes="{ value }">
            <span v-if="value" :title="value" class="block max-w-xs truncate text-sm text-text">{{ value }}</span>
          ...
          <template #cell-schedulable="{ row }">
            <button @click="handleToggleSchedulable(row)" :disabled="togglingSchedulable === row.id" class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50" :class="[row.schedulable ? 'bg-accent hover:bg-accent' : 'bg-muted hover:bg-muted']" :title="row.schedulable ? t('admin.accounts.schedulableEnabled') : t('admin.accounts.schedulableDisabled')">
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              <div v-if="row.proxy" class="flex items-center gap-2">
                <span class="text-sm text-text">{{ row.proxy.name }}</span>
                <span v-if="row.proxy.country_code" class="text-xs text-text-muted">
                  ({{ row.proxy.country_code }})
                </span>
              </div>
              <span v-else class="text-sm text-text-dim">-</span>
              <div v-if="row.proxy && row.proxy.expires_at" class="flex items-center gap-2 text-xs">
                <span class="text-text">{{ formatDateTime(row.proxy.expires_at) }}</span>
              ...
              <div v-if="row.proxy_fallback_origin_id" class="flex items-center gap-1">
                ...
                <button class="text-xs px-1.5 py-0.5 rounded border border-border text-text hover:bg-muted" @click="onRevertFallback(row)">{{ t('admin.accounts.revertProxy') }}</button>
              </div>
            </div>
          </template>
          ...
          <template #cell-last_used_at="{ value }">
            <span class="text-sm text-text-muted">{{ formatRelativeTime(value) }}</span>
          </template>
          ...
          <template #cell-expires_at="{ row, value }">
            <div class="flex flex-col items-start gap-1">
              <span class="text-sm text-text-muted">{{ formatExpiresAt(value) }}</span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button @click="handleEdit(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-text-dim transition-colors hover:bg-muted hover:text-accent">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                <span class="text-xs">{{ t('common.edit') }}</span>
              </button>
              <button @click="handleDelete(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-text-dim transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                <span class="text-xs">{{ t('common.delete') }}</span>
              </button>
              <button @click="openMenu(row, $event)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-text-dim transition-colors hover:bg-muted hover:text-text">
    ...
    <ConfirmDialog :show="showExportDataDialog" :title="t('admin.accounts.dataExport')" :message="t('admin.accounts.dataExportConfirmMessage')" :confirm-text="t('admin.accounts.dataExportConfirm')" :cancel-text="t('common.cancel')" @confirm="handleExportData" @cancel="showExportDataDialog = false">
      <label class="flex items-center gap-2 text-sm text-text">
        <input type="checkbox" class="h-4 w-4 rounded border-border text-accent focus:ring-accent" v-model="includeProxyOnExport" />
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `oc-router-account-${timestamp}.json`
...
// 所绑定代理的Valid期(逻辑同 /admin/proxies,见 utils/proxyExpiry)
...
.account-tools-menu-item {
  @apply flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm text-text transition-colors hover:bg-muted;
}
