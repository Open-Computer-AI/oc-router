  ...
  <AppLayout>
        ...
        >
          ...
          <template #cell-address="{ row }">
            <div class="flex items-center gap-1.5">
              ...
              <div class="relative">
                <button
                  type="button"
                  class="rounded p-0.5 text-text-dim hover:text-accent"
                ...
                <div
                  ...
                  class="absolute left-0 top-full z-50 mt-1 w-auto min-w-[180px] rounded-lg border border-border bg-bg-elevated py-1 shadow-lg"
                >
                  <button
                  ...
                  >
                    <span class="truncate font-mono text-text-muted">{{ fmt.label }}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <template #cell-auth="{ row }">
            <div v-if="row.username || row.password" class="flex items-center gap-1.5">
              <div class="flex flex-col text-xs">
                <span v-if="row.username" class="text-text">{{ row.username }}</span>
                <span v-if="row.password" class="font-mono text-text-muted">
              ...
              <button
                v-if="row.password"
                type="button"
                class="ml-1 rounded p-0.5 text-text-dim hover:text-text-muted"
          ...
          <template #cell-location="{ row }">
            <div class="flex items-center gap-2">
              ...
              />
              <span v-if="formatLocation(row)" class="text-sm text-text">
                {{ formatLocation(row) }}
              </span>
              <span v-else class="text-sm text-text-dim">-</span>
            </div>
          ...
          <template #cell-account_count="{ row, value }">
            <button
              v-if="(value || 0) > 0"
              type="button"
              class="inline-flex items-center rounded bg-muted px-2 py-0.5 text-xs font-medium text-accent hover:bg-muted"
          ...
          <template #cell-expiry="{ row }">
            <span v-if="!row.expires_at" class="text-sm text-text-dim">{{ t('admin.proxies.neverExpires') }}</span>
            <div v-else class="flex flex-col text-xs">
              <span class="text-text">{{ formatDateTime(row.expires_at) }}</span>
              <span :class="expiryBadgeClass(row)">{{ expiryLabel(row) }}</span>
            </div>
          </template>

          <template #cell-created_at="{ row }">
            <span class="text-xs text-text-muted">{{ formatDateTime(row.created_at) }}</span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-text-muted transition-colors hover:bg-success/10 hover:text-success disabled:cursor-not-allowed disabled:opacity-50"
              >
                <svg
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-text-muted transition-colors hover:bg-blue-50 hover:text-blue-600 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <svg
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button
              type="button"
              class="absolute right-3 top-1/2 -translate-y-1/2 text-text-dim hover:text-text-muted"
      ...
      <div v-else class="space-y-5">
        ...
        <!-- Parse Result -->
        <div v-if="batchParseResult.total > 0" class="rounded-lg bg-bg p-4">
      ...
      <div v-if="qualityReport" class="space-y-4">
        ...
        <div class="max-h-80 overflow-auto rounded-lg border border-border">
          <table class="min-w-full divide-y divide-border text-sm">
            <thead class="bg-bg text-xs uppercase text-text-muted">
              <tr>
            ...
            <tbody class="divide-y divide-border bg-bg-elevated">
              <tr v-for="item in qualityReport.items" :key="item.target">
                <td class="px-3 py-2 text-text">{{ qualityTargetLabel(item.target) }}</td>
                <td class="px-3 py-2">
                  <span class="badge" :class="qualityStatusClass(item.status)">{{ qualityStatusLabel(item.status) }}</span>
                </td>
                <td class="px-3 py-2 text-text-muted">{{ item.http_status ?? '-' }}</td>
                <td class="px-3 py-2 text-text-muted">
                  {{ typeof item.latency_ms === 'number' ? `${item.latency_ms}ms` : '-' }}
                </td>
                <td class="px-3 py-2 text-text-muted">
                  <span>{{ item.message || '-' }}</span>
                  <span v-if="item.cf_ray" class="ml-1 text-xs text-text-dim">(cf-ray: {{ item.cf_ray }})</span>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    >
      <div v-if="accountsLoading" class="flex items-center justify-center py-8 text-sm text-text-muted">
        <Icon name="refresh" size="md" class="mr-2 animate-spin" />
        {{ t('common.loading') }}
      </div>
      <div v-else-if="proxyAccounts.length === 0" class="py-6 text-center text-sm text-text-muted">
        ...
        <table class="min-w-full divide-y divide-border text-sm">
          ...
          <tbody class="divide-y divide-border bg-bg-elevated">
            <tr v-for="account in proxyAccounts" :key="account.id">
              <td class="px-4 py-2 font-medium text-text">{{ account.name }}</td>
              <td class="px-4 py-2">
