      ...
      <template #table>
        ...
        >
          ...
          <template #cell-aff_code="{ row }">
            <span class="font-mono text-sm text-text">{{ row.aff_code || '-' }}</span>
          </template>
          <template #cell-order="{ row }">
            <div class="space-y-0.5">
              <div class="font-mono text-sm text-text">#{{ row.order_id }}</div>
              <div class="max-w-56 truncate text-sm text-text-muted">{{ row.out_trade_no }}</div>
            </div>
          </template>
          ...
          <template #cell-pay_amount="{ row }">
            <span class="text-sm text-text">¥{{ formatAmount(row.pay_amount) }}</span>
          </template>
          ...
          <template #cell-created_at="{ row }">
            <span class="text-sm text-text">{{ formatDateTime(row.created_at) }}</span>
          </template>
        </DataTable>
      </template>
    ...
    >
      ...
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-lg border border-border bg-muted p-4">
          <div class="font-mono text-sm text-text">#{{ selectedOverview.user_id }}</div>
          <div class="mt-1 text-sm font-medium text-text">{{ selectedOverview.email || '-' }}</div>
          <div class="mt-0.5 text-sm text-text-muted">{{ selectedOverview.username || '-' }}</div>
        </div>
...
const UserCell = defineComponent({
  ...
  setup(cellProps, { emit }) {
    return () => h('div', { class: 'space-y-0.5' }, [
      h('div', { class: 'font-mono text-sm text-text' }, `#${cellProps.id}`),
      h(cellProps.clickable ? 'button' : 'div', {
        class: cellProps.clickable
          ? 'max-w-56 truncate text-left text-sm font-medium text-accent hover:underline'
          : 'max-w-56 truncate text-sm text-text',
        type: cellProps.clickable ? 'button' : undefined,
        onClick: cellProps.clickable ? () => emit('open', cellProps.id) : undefined,
      }, cellProps.email || '-'),
      h('div', { class: 'max-w-56 truncate text-sm text-text-muted' }, cellProps.username || '-'),
    ])
  },
})
  ...
  setup(amountProps) {
    return () => {
      ...
      if (value === null || value === undefined) {
        return h('span', { class: 'text-sm text-text-dim' }, '-')
      }
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () => h('div', { class: 'rounded-lg border border-border bg-bg-elevated p-3' }, [
      h('div', { class: 'text-sm text-text-muted' }, statProps.label),
      h('div', {
        class: statProps.mono
          ? 'mt-1 font-mono text-base font-semibold text-text'
          : 'mt-1 text-base font-semibold text-text',
      }, statProps.value),
    ])
  },
})
