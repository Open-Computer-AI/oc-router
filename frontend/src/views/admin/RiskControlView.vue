    ...
    <div class="space-y-6">
      ...
      <template v-else>
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 class="font-display text-2xl font-semibold text-text">{{ t('admin.riskControl.title') }}</h1>
            <p class="mt-1 text-sm text-text-muted">{{ t('admin.riskControl.description') }}</p>
          </div>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div
            ...
            :key="item.key"
            class="rounded-lg border border-border bg-bg-elevated px-4 py-3 shadow-sm"
          >
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0 flex-1">
                <div class="flex min-w-0 items-center justify-between gap-2">
                  <p class="truncate text-xs font-medium text-text-muted">{{ item.label }}</p>
                  <span
                    v-if="item.badge"
                ...
                <div class="mt-1 flex min-w-0 items-baseline gap-2">
                  <p class="truncate text-xl font-semibold leading-7 text-text">{{ item.value }}</p>
                  <p v-if="item.meta" class="truncate text-xs text-text-muted">{{ item.meta }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        >
          <div data-test="pre-block-sync-card" class="card">
            <div class="flex flex-col gap-4 border-b border-border px-6 py-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-text">{{ t('admin.riskControl.preBlockSyncStatus') }}</h2>
                <p class="mt-1 text-sm text-text-muted">{{ t('admin.riskControl.preBlockSyncHint') }}</p>
              </div>
              <span class="inline-flex w-fit items-center rounded-full bg-muted px-2.5 py-1 text-xs font-medium text-text-muted">
            ...
            <div class="p-6">
              <div data-test="pre-block-metric-grid" class="grid grid-cols-2 gap-3 md:grid-cols-3">
                ...
                >
                  ...
                  <p v-if="item.meta" class="mt-1 truncate text-xs text-text-muted">{{ item.meta }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div data-test="pre-block-api-key-load-card" class="card">
            <div class="flex flex-col gap-4 border-b border-border px-6 py-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-text">{{ t('admin.riskControl.preBlockAPIKeyLoad') }}</h2>
            ...
            <div class="p-6">
                  ...
                  <div class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                      <div class="flex min-w-0 items-center gap-2">
                        <span class="font-mono text-sm font-semibold text-text">#{{ item.index + 1 }}</span>
                        <span class="truncate font-mono text-sm text-text">{{ item.masked || '-' }}</span>
                    ...
                    <div class="grid grid-cols-4 gap-2 text-right text-xs text-text-muted sm:min-w-[280px]">
                      <div>
                      ...
                      <div>
                        ...
                        <p class="mt-1 text-sm font-semibold text-text">{{ formatNumber(item.total) }}</p>
                      </div>
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyAvgShort') }}</p>
                        <p class="mt-1 text-sm font-semibold text-text">{{ formatNumber(item.avg_latency_ms) }} ms</p>
                      </div>
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyLastShort') }}</p>
                        <p class="mt-1 text-sm font-semibold text-text">{{ formatNumber(item.last_latency_ms) }} ms</p>
                      </div>
                    </div>
                  </div>
              ...
              <p v-else class="rounded-lg bg-muted p-4 text-sm text-text-muted">
        ...
        <div v-if="showWorkerRuntimeCard" class="card">
          <div class="flex flex-col gap-4 border-b border-border px-6 py-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 class="text-lg font-semibold text-text">{{ t('admin.riskControl.workerStatus') }}</h2>
              <p class="mt-1 text-sm text-text-muted">{{ t('admin.riskControl.workerStatusHint') }}</p>
            </div>
          ...
          <div class="grid grid-cols-1 gap-6 p-6 xl:grid-cols-[minmax(0,360px)_1fr]">
            <div class="space-y-4">
              <div class="rounded-lg border border-border p-4">
                <div class="flex items-center justify-between gap-3">
                  <div>
                    <p class="text-sm font-medium text-text">{{ t('admin.riskControl.queueUsage') }}</p>
                    <p class="mt-1 text-xs text-text-muted">
                      {{ formatNumber(status?.queue_length ?? 0) }} / {{ formatNumber(status?.queue_size ?? configForm.queue_size) }}
                    </p>
                  </div>
                  <span class="text-sm font-semibold text-text">{{ queueUsagePercent }}</span>
                </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="rounded-lg bg-muted p-4">
                  <p class="text-xs text-text-muted">{{ t('admin.riskControl.activeWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-text">{{ status?.active_workers ?? 0 }}</p>
                </div>
                <div class="rounded-lg bg-success/10 p-4">
                  <p class="text-xs text-text-muted">{{ t('admin.riskControl.idleWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-success">{{ status?.idle_workers ?? configForm.worker_count }}</p>
                </div>
                <div class="rounded-lg bg-muted p-4">
                  <p class="text-xs text-text-muted">{{ t('admin.riskControl.processed') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-text">{{ formatNumber(status?.processed ?? 0) }}</p>
                </div>
                <div class="rounded-lg bg-muted p-4">
                  <p class="text-xs text-text-muted">{{ t('admin.riskControl.droppedErrors') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-text">{{ formatNumber((status?.dropped ?? 0) + (status?.errors ?? 0)) }}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              <div class="mb-3 flex items-center justify-between gap-3">
                <div>
                  <p class="text-sm font-medium text-text">{{ t('admin.riskControl.workerPool') }}</p>
                  <p class="mt-1 text-xs text-text-muted">
                ...
                <span class="inline-flex items-center rounded-full bg-muted px-2.5 py-1 text-xs font-medium text-text-muted">
                  {{ modeLabel(status?.mode ?? configForm.mode) }}
                </span>
              </div>
        ...
        <div class="card">
          <div class="flex flex-col gap-4 border-b border-border px-6 py-4">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-text">{{ t('admin.riskControl.records') }}</h2>
                <p class="mt-1 text-sm text-text-muted">{{ t('admin.riskControl.recordsHint') }}</p>
              </div>
            ...
            <div class="flex flex-col gap-2 rounded-lg border border-border bg-muted px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
              <div class="flex min-w-0 items-center gap-2 text-sm text-text">
                <Icon name="filter" size="sm" class="flex-shrink-0 text-text-dim" />
                <span class="font-medium">{{ t('admin.riskControl.modelFilter') }}</span>
                <span class="truncate text-text-muted">{{ modelFilterSummary }}</span>
              </div>
              <div v-if="modelFilterPreviewModels.length > 0" class="flex flex-wrap gap-1.5">
                <span
                  v-for="model in modelFilterPreviewModels"
                  :key="model"
                  class="inline-flex max-w-[180px] items-center truncate rounded-md bg-bg-elevated px-2 py-1 font-mono text-xs text-text-muted shadow-sm"
                >
                  {{ model }}
                </span>
                <span v-if="hiddenModelFilterModelCount > 0" class="inline-flex rounded-md bg-bg-elevated px-2 py-1 text-xs text-text-muted shadow-sm">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-border">
              <thead class="bg-bg">
                <tr>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted">{{ t('admin.riskControl.table.time') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted">{{ t('admin.riskControl.table.group') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted">{{ t('admin.riskControl.table.user') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted">{{ t('admin.riskControl.table.apiKey') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted">{{ t('admin.riskControl.table.endpoint') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted">{{ t('admin.riskControl.table.result') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted">{{ t('admin.riskControl.table.highest') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted">{{ t('admin.riskControl.table.actionMeta') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted">{{ t('admin.riskControl.table.latency') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted">{{ t('admin.riskControl.table.input') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-border bg-bg-elevated">
                <tr v-if="logsLoading">
                  <td colspan="10" class="px-5 py-12 text-center text-sm text-text-muted">{{ t('common.loading') }}</td>
                </tr>
                <tr v-else-if="logs.length === 0">
                  <td colspan="10" class="px-5 py-12 text-center text-sm text-text-muted">{{ t('admin.riskControl.emptyLogs') }}</td>
                </tr>
                <template v-else>
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-muted">
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-text">{{ formatDateTime(row.created_at) }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-text">{{ row.group_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-text">
                      <div>{{ row.user_email || '-' }}</div>
                      <div v-if="row.user_id" class="text-xs text-text-dim">UID {{ row.user_id }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-text">{{ row.api_key_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-text">
                      <div>{{ row.endpoint || '-' }}</div>
                      <div class="text-xs text-text-dim">{{ row.provider || '-' }} / {{ row.model || '-' }}</div>
                    </td>
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-text">
                      ...
                      <div class="text-xs text-text-dim">{{ percent(row.highest_score) }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-text">
                      ...
                      <button
                        ...
                        type="button"
                        class="mt-2 inline-flex items-center gap-1 rounded-md border border-success/30 bg-success/10 px-2 py-1 text-xs font-medium text-success transition-colors hover:bg-success/20 disabled:cursor-not-allowed disabled:opacity-60"
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-text">
                      ...
                      <div v-if="row.queue_delay_ms !== null && row.queue_delay_ms !== undefined" class="text-xs text-text-dim">
                        {{ t('admin.riskControl.queueDelay', { ms: row.queue_delay_ms }) }}
                      </div>
                    </td>
                    <td class="w-[320px] max-w-sm px-5 py-4 text-sm text-text">
                      <button
                        type="button"
                      ...
                      >
                        ...
                        <Icon name="eye" size="xs" class="flex-shrink-0 text-text-dim transition-colors group-hover:text-accent" />
                      </button>
                    </td>
                  </tr>
                </template>
      ...
      <BaseDialog :show="settingsOpen" :title="t('admin.riskControl.settingsTitle')" width="extra-wide" @close="settingsOpen = false">
        <div class="space-y-6">
          <div class="flex gap-2 overflow-x-auto border-b border-border pb-3">
            <button
              v-for="tab in settingsTabs"
              :key="tab.id"
              type="button"
              class="inline-flex whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium transition-colors"
              :class="activeSettingsTab === tab.id ? 'bg-accent-muted text-accent' : 'text-text-muted hover:bg-muted hover:text-text'"
          ...
          <div v-if="activeSettingsTab === 'basic'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              <div class="flex items-center justify-between rounded-lg border border-border p-4">
                <div>
                  <p class="text-sm font-medium text-text">{{ t('admin.riskControl.enabled') }}</p>
                  <p class="mt-1 text-xs text-text-muted">{{ t('admin.riskControl.enabledHint') }}</p>
                </div>
              ...
              <div>
                ...
                <p class="mt-2 text-xs leading-5 text-text-muted">{{ modeDescription(configForm.mode) }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <span class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-text-dim">%</span>
                </div>
              </div>
            </div>

            <div class="overflow-hidden rounded-xl border border-border bg-bg-elevated shadow-sm">
              <div class="flex flex-col gap-4 border-b border-border bg-muted px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-start gap-3">
                  <span class="mt-0.5 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-accent-muted text-accent">
                    <Icon name="key" size="md" />
                  </span>
                  <div>
                    <label class="text-sm font-semibold text-text">{{ t('admin.riskControl.apiKeys') }}</label>
                    <p class="mt-1 max-w-3xl text-xs leading-5 text-text-muted">
              ...
              <div class="grid grid-cols-1 gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,440px)]">
                <div class="space-y-3">
                  <div class="flex flex-col gap-2 rounded-lg border border-border bg-muted p-2 sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-xs leading-5 text-text-muted">
                      <span class="font-medium text-text">{{ t('admin.riskControl.apiKeysWriteMode') }}</span>
                      <span class="ml-2">{{ apiKeysModeHint }}</span>
                    </div>
                    <div class="inline-flex rounded-lg bg-bg-elevated p-1 shadow-sm">
                      <button
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'append' ? 'bg-accent text-white shadow-sm' : 'text-text-muted hover:bg-muted'"
                      ...
                      <button
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'replace' ? 'bg-warning text-white shadow-sm' : 'text-text-muted hover:bg-muted'"
                        :disabled="configForm.clear_api_key"
                  ...
                  <div class="flex flex-wrap items-center gap-2 text-xs text-text-muted">
                    ...
                    <span v-if="configForm.clear_api_key" class="inline-flex rounded-md bg-danger/10 px-2 py-1 text-danger">
                      {{ t('admin.riskControl.apiKeyWillClear') }}
                    </span>
                    <span v-else-if="pendingDeletedApiKeyCount > 0" class="inline-flex rounded-md bg-warning/10 px-2 py-1 text-warning">
                      {{ t('admin.riskControl.apiKeyPendingDeleteCount', { count: pendingDeletedApiKeyCount }) }}
                    </span>
                    <span v-if="configForm.api_keys_mode === 'replace'" class="inline-flex rounded-md bg-warning/10 px-2 py-1 text-warning">
                      {{ t('admin.riskControl.apiKeysReplaceWarning') }}
                    </span>
                  </div>

                  <div class="rounded-lg border border-border bg-muted p-3" @paste="handleModerationImagePaste">
                    <div class="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <p class="text-sm font-semibold text-text">{{ t('admin.riskControl.auditTestInput') }}</p>
                        <p class="mt-1 text-xs text-text-muted">{{ t('admin.riskControl.auditTestInputHint') }}</p>
                      </div>
                      <button
                        v-if="moderationTestPrompt || moderationTestImages.length > 0 || moderationTestResult"
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-text-muted hover:bg-bg-elevated hover:text-text"
                    ...
                    ></textarea>
                    <div
                      class="mt-3 rounded-lg border border-dashed border-border bg-bg-elevated p-3"
                      @dragover.prevent
                      @drop.prevent="handleModerationImageDrop"
                    >
                      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-start gap-2">
                          <Icon name="upload" size="md" class="mt-0.5 text-text-dim" />
                          <div>
                            <p class="text-sm font-medium text-text">{{ t('admin.riskControl.auditTestImages') }}</p>
                            <p class="mt-1 text-xs text-text-muted">{{ t('admin.riskControl.auditTestImagesHint') }}</p>
                          </div>
                        </div>
                ...
                <div class="rounded-lg border border-border bg-muted p-3">
                  <div class="mb-3 flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <p class="text-sm font-semibold text-text">{{ t('admin.riskControl.apiKeyHealth') }}</p>
                      <p class="mt-1 text-xs text-text-muted">{{ t('admin.riskControl.apiKeyFreezeRule') }}</p>
                    </div>
                    <span class="inline-flex shrink-0 items-center whitespace-nowrap rounded-full bg-bg-elevated px-2 py-0.5 text-[11px] font-medium leading-5 text-text-muted shadow-sm">
                      {{ t('admin.riskControl.apiKeyRows', { count: apiKeyRows.length }) }}
                    </span>
                  </div>

                  <div v-if="apiKeyRows.length === 0" class="flex min-h-32 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-bg-elevated px-4 py-6 text-center">
                    <Icon name="infoCircle" size="lg" class="text-text-dim" />
                    <p class="mt-2 text-sm font-medium text-text">{{ t('admin.riskControl.apiKeyHealthEmpty') }}</p>
                    <p class="mt-1 text-xs text-text-muted">{{ t('admin.riskControl.apiKeyHealthEmptyHint') }}</p>
                  </div>
                  <div v-else class="space-y-2">
                    <div class="space-y-2" :class="apiKeyRowsExpanded ? 'max-h-72 overflow-y-auto pr-1' : ''">
                      <div
                        ...
                        class="rounded-lg border bg-bg-elevated p-2.5 shadow-sm"
                        :class="isStoredApiKeyPendingDelete(row) ? 'border-warning/30 opacity-70' : 'border-border'"
                      >
                        <div class="flex items-start justify-between gap-2">
                          <div class="min-w-0">
                            <div class="flex min-w-0 flex-wrap items-center gap-2">
                              <span class="truncate font-mono text-sm font-semibold text-text">{{ row.masked || '-' }}</span>
                              <span
                                class="inline-flex rounded-md px-1.5 py-0.5 text-[11px] font-medium"
                                :class="row.configured ? 'bg-accent-muted text-accent' : 'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300'"
                              >
                            ...
                            <p class="mt-1 text-xs leading-5 text-text-muted">{{ apiKeyStatusMeta(row) }}</p>
                          </div>
                          <div class="flex flex-shrink-0 items-center gap-1.5">
                            ...
                            <button
                              ...
                              type="button"
                              class="inline-flex h-7 w-7 items-center justify-center rounded-md text-text-dim transition-colors hover:bg-muted hover:text-text"
                        ...
                        <p v-if="row.last_error" class="mt-1.5 rounded-md bg-warning/10 px-2 py-1.5 text-xs leading-5 text-warning">
                          {{ row.last_error }}
                        </p>
                      </div>
                    </div>

                    <div v-if="canToggleApiKeyRows" class="flex items-center justify-between gap-3 rounded-lg border border-dashed border-border bg-bg-elevated px-3 py-2 text-xs text-text-muted">
                      ...
                      <button
                        type="button"
                        class="inline-flex shrink-0 items-center gap-1 rounded-md px-2 py-1 font-medium text-accent transition-colors hover:bg-accent-muted hover:text-accent"
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-lg border border-border bg-bg-elevated p-3">
                    <div class="flex items-start justify-between gap-3">
                      <div>
                        <p class="text-sm font-semibold text-text">{{ t('admin.riskControl.auditTestResult') }}</p>
                        <p class="mt-1 text-xs text-text-muted">
                      ...
                      <span class="inline-flex rounded-full px-2 py-1 text-xs font-medium" :class="moderationTestResult.flagged ? 'bg-danger/10 text-danger' : 'bg-success/10 text-success'">
                    ...
                    <div class="mt-3">
                      <div class="mb-2 flex items-center justify-between text-xs text-text-muted">
                        ...
                        <span class="font-semibold text-text">{{ percent(moderationTestResult.composite_score) }}</span>
                      </div>
                    ...
                    <div class="mt-3 max-h-52 space-y-2 overflow-y-auto pr-1">
                      <div v-for="score in moderationScoreRows" :key="score.category">
                        <div class="mb-1 flex items-center justify-between gap-3 text-xs">
                          <span class="truncate text-text-muted">{{ score.category }}</span>
                          <span class="font-mono text-text-muted">{{ percent(score.score) }} / {{ percent(score.threshold) }}</span>
                        </div>
                        <div class="h-1.5 overflow-hidden rounded-full bg-muted">
                          <div class="h-full rounded-full" :class="score.hit ? 'bg-danger' : 'bg-accent'" :style="{ width: percentWidth(score.score) }"></div>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div v-else-if="activeSettingsTab === 'scope'" class="space-y-5">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-semibold text-text">{{ t('admin.riskControl.groupScope') }}</h3>
                <p class="mt-1 text-sm text-text-muted">{{ t('admin.riskControl.groupScopeHint') }}</p>
              </div>
              <div class="inline-flex rounded-lg bg-muted p-1">
                <button
                  type="button"
                  class="rounded-md px-3 py-1.5 text-sm font-medium transition-colors"
                  :class="configForm.all_groups ? 'bg-bg-elevated text-text shadow-sm' : 'text-text-muted'"
            ...
            <div v-if="!configForm.all_groups" class="space-y-4">
              <div class="relative">
                <Icon name="search" size="sm" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-text-dim" />
              ...
              <div class="grid max-h-[420px] grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-3">
                <button
                  ...
                  :class="isGroupSelected(group.id) ? 'border-accent/30 bg-accent-muted' : 'border-border hover:bg-muted'"
                  @click="toggleGroup(group.id)"
                >
                  <span class="min-w-0">
                    <span class="block truncate text-sm font-semibold text-text">{{ group.name }}</span>
                    <span class="mt-1 inline-flex rounded-md bg-muted px-2 py-0.5 text-xs text-text-muted">{{ group.platform }}</span>
                  </span>
                  <span
                    class="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full border"
                    :class="isGroupSelected(group.id) ? 'border-accent bg-accent text-white' : 'border-border text-transparent'"
                  >
                    <Icon name="check" size="xs" :stroke-width="2" />
                  </span>
                </button>
                <p v-if="filteredGroups.length === 0" class="text-sm text-text-muted">{{ t('admin.riskControl.noGroups') }}</p>
              </div>
            </div>
            ...
            <div class="space-y-4 rounded-lg border border-border p-4">
              <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h3 class="text-base font-semibold text-text">{{ t('admin.riskControl.modelFilter') }}</h3>
                  <p class="mt-1 text-sm text-text-muted">{{ t('admin.riskControl.modelFilterHint') }}</p>
                </div>
                <span class="inline-flex w-fit rounded-md bg-muted px-2.5 py-1 text-xs font-medium text-text-muted">
                  {{ modelFilterSummary }}
                </span>
              </div>
              ...
              <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
                <button
                  ...
                  :class="configForm.model_filter_type === option.value
                    ? 'border-accent/30 bg-accent-muted text-text shadow-sm'
          ...
          <div v-else-if="activeSettingsTab === 'runtime'" class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="flex items-center justify-between rounded-lg border border-border p-4 lg:col-span-2">
              <div>
                <p class="text-sm font-medium text-text">{{ t('admin.riskControl.recordNonHits') }}</p>
                <p class="mt-1 text-xs text-text-muted">{{ t('admin.riskControl.recordNonHitsHint') }}</p>
              </div>
              <Toggle v-model="configForm.record_non_hits" />
            </div>
            <div class="space-y-4 rounded-lg border border-border p-4 lg:col-span-2">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <p class="text-sm font-medium text-text">{{ t('admin.riskControl.preHashCheck') }}</p>
                  <p class="mt-1 text-xs text-text-muted">{{ t('admin.riskControl.preHashCheckHint') }}</p>
                </div>
              ...
              <div class="rounded-lg bg-muted p-3">
                <div class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                  <div>
                    ...
                    <p class="mt-1 text-xs text-text-muted">{{ t('admin.riskControl.flaggedHashHint') }}</p>
                  </div>
                  <button
                    type="button"
                    class="btn btn-secondary inline-flex items-center justify-center gap-2 text-danger"
          ...
          <div v-else-if="activeSettingsTab === 'response'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              ...
              <div class="flex items-center justify-between rounded-lg border border-border p-4">
                <div>
                  <p class="text-sm font-medium text-text">{{ t('admin.riskControl.emailOnHit') }}</p>
                  <p class="mt-1 text-xs text-text-muted">{{ t('admin.riskControl.emailOnHitHint') }}</p>
                </div>
                <Toggle v-model="configForm.email_on_hit" />
              </div>
              <div class="flex items-center justify-between rounded-lg border border-border p-4">
                <div>
                  <p class="text-sm font-medium text-text">{{ t('admin.riskControl.autoBan') }}</p>
                  <p class="mt-1 text-xs text-text-muted">{{ t('admin.riskControl.autoBanHint') }}</p>
                </div>
          ...
          <div v-else-if="activeSettingsTab === 'riskThresholds'" class="space-y-5">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-semibold text-text">{{ t('admin.riskControl.riskThresholds') }}</h3>
                <p class="mt-1 text-sm text-text-muted">{{ t('admin.riskControl.riskThresholdsHint') }}</p>
              </div>
              <button
                type="button"
                class="btn btn-secondary inline-flex items-center justify-center gap-2"
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              ...
              >
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                    <label class="block truncate text-sm font-semibold text-text" :for="`risk-threshold-${row.category}`">
                      {{ row.category }}
                    </label>
                    <p class="mt-1 text-xs text-text-muted">
                  ...
                  <span class="inline-flex shrink-0 rounded-md bg-bg-elevated px-2 py-1 font-mono text-xs font-medium text-text-muted shadow-sm">
          ...
          <div v-else-if="activeSettingsTab === 'keywords'" class="space-y-5">
            ...
            >
              ...
              <div class="text-sm leading-6">
                ...
                <p class="mt-1 text-xs text-text-muted">{{ keywordNotice.description }}</p>
              </div>
            </div>
            ...
            <div>
              <div class="mb-2 flex items-center justify-between">
                ...
                <span class="inline-flex rounded-md bg-muted px-2 py-1 text-xs text-text-muted">
          ...
          <div v-else class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="rounded-lg border border-border p-4 text-sm text-text-muted lg:col-span-2">
              <div class="flex flex-wrap items-center gap-3">
                <Icon name="database" size="md" class="text-text-dim" />
        ...
        <div v-if="inputDetailRow" class="space-y-5">
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg border border-border bg-muted p-4">
              <p class="text-xs font-medium text-text-muted">{{ t('admin.riskControl.table.time') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-text">{{ formatDateTime(inputDetailRow.created_at) }}</p>
            </div>
            <div class="rounded-lg border border-border bg-muted p-4">
              <p class="text-xs font-medium text-text-muted">{{ t('admin.riskControl.table.user') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-text">{{ inputDetailRow.user_email || '-' }}</p>
            </div>
            <div class="rounded-lg border border-border bg-muted p-4">
              <p class="text-xs font-medium text-text-muted">{{ t('admin.riskControl.table.result') }}</p>
            ...
            <div class="rounded-lg border border-border bg-muted p-4">
              <p class="text-xs font-medium text-text-muted">{{ t('admin.riskControl.table.highest') }}</p>
          ...
          <div class="rounded-xl border border-border bg-bg-elevated p-4 shadow-sm">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p class="text-sm font-semibold text-text">{{ t('admin.riskControl.inputDetailContent') }}</p>
                <p class="mt-1 text-xs text-text-muted">
...
const keywordNoticeTones = {
  info: {
    ...
    toneClass: 'border-accent/20 bg-accent-muted',
  ...
  warning: {
    ...
    iconClass: 'mt-0.5 flex-shrink-0 text-warning',
...
function resultBadgeClass(row: ContentModerationLog): string {
  if (row.action === 'block' || row.action === 'keyword_block') return 'bg-danger/10 text-danger'
  if (row.action === 'error' || row.error) return 'bg-warning/10 text-warning'
...
function apiKeyStatusBadgeClass(statusValue: ContentModerationAPIKeyStatus['status']): string {
  const classes: Record<ContentModerationAPIKeyStatus['status'], string> = {
    ...
    unknown: 'bg-muted text-text-muted',
  }
