      ...
      <template #table>
        ...
        >
          ...
          <template #cell-group_count="{ row }">
            <span
              class="inline-flex items-center rounded bg-muted px-2 py-0.5 text-xs font-medium text-text"
            >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-text-muted transition-colors hover:bg-red-50 hover:text-danger"
              >
    ...
    >
      <div class="channel-dialog-body">
        ...
        <div class="flex items-center border-b border-border flex-shrink-0 -mx-4 sm:-mx-6 px-4 sm:px-6 -mt-3 sm:-mt-4">
          ...
          >
            {{ t('admin.channels.form.basicSettings', '基础Settings') }}
          </button>
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            ...
            <div>
              ...
              <p class="mt-1 ml-6 text-xs text-text-dim">
          ...
          >
            ...
            <div>
              <label class="input-label text-xs">
                ...
                <span v-if="section.group_ids.length > 0" class="ml-1 font-normal text-text-dim">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length }, `已选 ${section.group_ids.length} 个`) }})
                </span>
              </label>
              <div class="max-h-40 overflow-auto rounded-lg border border-border bg-bg p-2">
                <div v-if="groupsLoading" class="py-2 text-center text-xs text-text-muted">
                  {{ t('common.loading', 'Loading...') }}
                </div>
                <div v-else-if="getGroupsForPlatform(section.platform).length === 0" class="py-2 text-center text-xs text-text-muted">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  <label
                    ...
                    :key="group.id"
                    class="inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs transition-colors hover:bg-muted"
                    :class="[
                      section.group_ids.includes(group.id) ? 'bg-accent-muted border-accent' : '',
                  ...
                  >
                    <input
                      ...
                      class="h-3 w-3 rounded border-border text-accent focus:ring-accent"
                    ...
                    <span class="text-[10px] text-text-dim">{{ group.account_count || 0 }}</span>
                    <span
            ...
            <div v-if="section.platform === 'anthropic' && webSearchGlobalEnabled" class="border-t border-border pt-3">
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p class="mt-0.5 text-[11px] text-danger">
            ...
            <div v-if="section.platform === 'openai'" class="border-t border-border pt-3">
              <div class="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p class="mt-0.5 text-[11px] text-warning">
            ...
            <div>
              <div class="mb-1 flex items-center justify-between">
                ...
                <button type="button" @click="addMappingEntry(sIdx)" class="text-xs text-accent hover:text-accent">
                  + {{ t('common.add', 'Add') }}
                </button>
              </div>
              <div
                v-if="Object.keys(section.model_mapping).length === 0"
                class="rounded border border-dashed border-border p-2 text-center text-xs text-text-dim"
              >
              ...
              <div v-else class="space-y-1">
                ...
                >
                  ...
                  <button
                    ...
                    class="rounded p-0.5 text-text-dim hover:text-danger"
                  >
                    <Icon name="trash" size="sm" />
                  </button>
                </div>
              ...
              <div class="mb-1 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  <button
                    ...
                    class="text-xs text-text-muted hover:text-accent disabled:opacity-50"
                  >
                    {{ syncingPlatform === section.platform ? t('admin.channels.form.syncingModels') : t('admin.channels.form.syncLatestModels') }}
                  </button>
                  <button type="button" @click="addPricingEntry(sIdx)" class="text-xs text-accent hover:text-accent">
                    + {{ t('common.add', 'Add') }}
                  </button>
                </div>
              </div>
            ...
            <div class="mt-4 border-t border-border pt-4 space-y-3">
              <div class="flex items-center justify-between">
                ...
                <button
                  ...
                  class="rounded-lg border border-accent px-3 py-1 text-xs font-medium text-accent hover:bg-accent-muted"
                >
              ...
              <p
                ...
                class="text-xs italic text-text-dim"
              >
              ...
              >
                <div class="flex items-center justify-between">
                  <input
                    ...
                    class="bg-transparent text-sm font-medium text-text placeholder-text-dim outline-none"
                  />
                ...
                <div>
                  <label class="text-xs text-text-muted">{{ t('admin.channels.form.ruleGroups') }}</label>
                  <div class="mt-1 flex flex-wrap gap-1">
                    <label
                      ...
                      :class="rule.group_ids.includes(gid)
                        ...
                        : 'border-border hover:bg-muted'"
                    >
                      <input type="checkbox" :checked="rule.group_ids.includes(gid)" class="h-3 w-3 rounded border-border text-accent focus:ring-accent" @change="rule.group_ids.includes(gid) ? rule.group_ids.splice(rule.group_ids.indexOf(gid), 1) : rule.group_ids.push(gid)" />
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getGroupNameById(gid) }}</span>
                    </label>
                  </div>
                  <p v-if="section.group_ids.length === 0" class="mt-1 text-xs text-text-dim">
                    {{ t('admin.channels.form.noGroupsInChannel') }}
                  </p>
                </div>

                <div>
                  <label class="text-xs text-text-muted">{{ t('admin.channels.form.ruleAccounts') }}</label>
                  <!-- Selected account chips -->
                  <div class="mt-1 flex flex-wrap gap-1">
                    <span
                      v-for="accountId in rule.account_ids"
                      :key="accountId"
                      class="inline-flex items-center gap-1 rounded-md border border-accent bg-accent-muted px-2 py-0.5 text-xs"
                    >
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getRuleAccountLabel(accountId) }}</span>
                      <button type="button" @click="removeRuleAccount(rule, accountId)" class="text-text-dim hover:text-danger">
                        <Icon name="x" size="xs" />
                      </button>
                    </span>
                  </div>
                  ...
                  <div class="relative mt-1 rule-account-search-container">
                    ...
                    <div
                      ...
                      class="absolute z-50 mt-1 max-h-48 w-full overflow-auto rounded-lg border border-border bg-bg-elevated shadow-lg"
                    >
                      <button
                      ...
                      >
                        ...
                        <span class="ml-2 text-xs text-text-dim">#{{ account.id }}</span>
                      </button>
                    </div>
                  </div>
                  <p class="mt-1 text-xs text-text-dim">
                ...
                <div>
                  <div class="mb-1 flex items-center justify-between">
                    <label class="text-xs text-text-muted">{{ t('admin.channels.form.ruleModelPricing') }}</label>
                    <button type="button" @click="addRulePricingEntry(sIdx, ruleIndex)" class="text-xs text-accent hover:text-accent">
                      + {{ t('common.add') }}
                    </button>
                  </div>
                  <div v-if="rule.pricing.length === 0" class="rounded border border-dashed border-border p-2 text-center text-xs text-text-dim">
...
async function handleSubmit() {
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    if (section.group_ids.length === 0) {
      ...
      appStore.showError(t('admin.channels.noGroupsSelected', { platform: platformLabel }, `${platformLabel} 平台未选择分组，请至少选择一个分组或Disable该平台`))
    ...
    for (const entry of section.model_pricing) {
      if (entry.models.length === 0) {
        const platformLabel = t('admin.groups.platforms.' + section.platform, section.platform)
        appStore.showError(t('admin.channels.emptyModelsInPricing', { platform: platformLabel }, `${platformLabel} 平台下有定价条目未Add模型，请Add模型或Delete该条目`))
        activeTab.value = section.platform
        return
      }
    }
  ...
  // 校验 per_request/image 模式必须有价格 (只校验Enable的平台)
  for (const section of form.platforms.filter(s => s.enabled)) {
    for (const entry of section.model_pricing) {
      if (entry.models.length === 0) continue
      if ((entry.billing_mode === 'per_request' || entry.billing_mode === 'image') &&
        ...
        appStore.showError(t('admin.channels.form.perRequestPriceRequired', '按次/图片计费模式必须Settings默认价格或至少一个计费层级'))
        return
      }
    }
  }
...
.channel-tab-inactive {
  @apply border-transparent text-text-muted hover:text-text hover:border-border;
}
</style>
