      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-border">
        <table class="min-w-full divide-y divide-border">
          ...
          <tbody class="divide-y divide-border bg-bg-elevated">
            <tr v-for="profile in profiles" :key="profile.id" class="hover:bg-muted">
              <td class="px-3 py-2">
                <div class="font-medium text-text text-sm">{{ profile.name }}</div>
              </td>
              <td class="px-3 py-2">
                <div v-if="profile.description" class="text-sm text-text-muted max-w-xs truncate">
                  {{ profile.description }}
                </div>
                <div v-else class="text-xs text-text-dim">—</div>
              </td>
              <td class="px-3 py-2">
                <Icon
        ...
        <div>
          ...
          <div class="mt-1 flex items-center gap-2">
            ...
            <p class="text-xs text-text-muted">
              ...
              <a href="https://tls.oc-router.org" target="_blank" rel="noopener noreferrer" class="text-accent hover:text-accent underline">{{ t('admin.tlsFingerprintProfiles.form.openCollector') }}</a>
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          <button
            ...
            :class="[
              ...
              form.enable_grease ? 'bg-accent' : 'bg-muted'
            ]"
          >
            <span
              :class="[
