<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div
        v-if="showAddForm"
        class="rounded-xl border border-accent bg-accent-muted p-4"
      >
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div>
            <label class="mb-1 flex items-center gap-1 text-xs font-medium text-text-muted">
              {{ t('admin.scheduledTests.cronExpression') }}
              <HelpTooltip>
                <template #trigger>
                  <span class="inline-flex h-4 w-4 cursor-help items-center justify-center rounded-full border border-text-dim/70 text-[10px] font-semibold text-text-dim transition-colors hover:border-accent hover:text-accent">
                    ?
                  </span>
                </template>
        ...
        <div class="mt-3 flex justify-end gap-2">
          <button
            ...
            class="rounded-lg bg-muted px-3 py-1.5 text-sm font-medium text-text transition-colors hover:bg-muted"
          >
            {{ t('common.cancel') }}
          </button>
          <button
            ...
            class="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-accent disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        <Icon name="refresh" size="md" class="animate-spin text-text-dim" :stroke-width="2" />
        <span class="ml-2 text-sm text-text-muted">{{ t('common.loading') }}...</span>
      </div>
      ...
      >
        <Icon name="calendar" size="lg" class="mx-auto mb-2 text-text-dim" :stroke-width="1.5" />
        <p class="text-sm text-text-muted">
      ...
      <div v-else class="space-y-3">
        <div
          v-for="plan in plans"
          :key="plan.id"
          class="rounded-xl border border-border bg-bg-elevated transition-all"
        >
          <!-- Plan Header -->
          <div
          ...
          >
            ...
            <div class="flex items-center gap-3">
              <!-- Last Run -->
              <div v-if="plan.last_run_at" class="hidden text-right text-xs text-text-muted sm:block">
                <div>{{ t('admin.scheduledTests.lastRun') }}</div>
                <div>{{ formatDateTime(plan.last_run_at) }}</div>
              </div>

              <!-- Next Run -->
              <div v-if="plan.next_run_at" class="hidden text-right text-xs text-text-muted sm:block">
              ...
              <div class="flex items-center gap-1" @click.stop>
                <button
                  ...
                  class="rounded-lg p-1.5 text-text-dim transition-colors hover:bg-blue-50 hover:text-blue-500 dark:hover:bg-blue-900/20"
                ...
                <button
                  ...
                  class="rounded-lg p-1.5 text-text-dim transition-colors hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20"
          ...
          >
            ...
            <div v-if="loadingResults" class="flex items-center justify-center py-4">
              <Icon name="refresh" size="sm" class="animate-spin text-text-dim" :stroke-width="2" />
              <span class="ml-2 text-xs text-text-muted">{{ t('common.loading') }}...</span>
            </div>
              ...
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    ...
                    <!-- Latency -->
                    <span v-if="result.latency_ms > 0" class="text-xs text-text-muted">
                ...
                <div v-if="result.error_message" class="mt-2">
                  <div
                    class="cursor-pointer text-xs font-medium text-danger"
                ...
                <div v-else-if="result.response_text" class="mt-2">
                  ...
                  <pre
                    v-if="expandedResultIds.has(result.id)"
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded bg-muted p-2 text-xs text-text"
