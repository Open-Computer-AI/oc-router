<template>
  <div class="card p-4">
    ...
    <div
      v-if="!users?.length"
      class="flex h-32 items-center justify-center text-sm text-text-muted"
    >
    ...
    <div v-else class="space-y-2">
      <div
        ...
        :key="user.user_id"
        class="flex items-center justify-between rounded-lg px-3 py-2 hover:bg-muted"
      >
...
function rankClass(idx: number): string {
  ...
  if (idx === 1) return 'bg-muted text-text-muted'
