    ...
    <form id="refund-form" @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div class="rounded-lg bg-muted p-3">
        <div class="flex justify-between text-sm">
          ...
          <span class="font-mono text-text">#{{ order?.id }}</span>
        </div>
        <div class="mt-1 flex justify-between text-sm">
          <span class="text-text-muted">{{ t('payment.orders.creditedAmount') }}</span>
          <span class="font-medium text-text">{{ order?.order_type === 'balance' ? '$' : '¥' }}{{ order?.amount?.toFixed(2) }}</span>
        </div>
        <div class="mt-1 flex justify-between text-sm">
          <span class="text-text-muted">{{ t('payment.orders.payAmount') }}</span>
          <span class="font-medium text-text">¥{{ order?.pay_amount?.toFixed(2) }}</span>
        </div>
        <div v-if="actuallyRefunded > 0" class="mt-1 flex justify-between text-sm">
          <span class="text-text-muted">{{ t('payment.admin.alreadyRefunded') }}</span>
      ...
      <div>
        <div class="flex items-center gap-2">
          ...
          />
          <label for="deduct-balance" class="text-sm text-text">
            {{ t('payment.admin.deductBalance') }}
          </label>
          <span class="text-xs text-text-muted">{{ t('payment.admin.deductBalanceHint') }}</span>
        </div>
        ...
        <div v-if="form.deduct_balance && userBalance != null" class="mt-3 grid grid-cols-2 gap-3">
          <div class="rounded-lg bg-muted p-3 text-sm">
            <div class="text-text-muted">{{ t('payment.admin.userBalance') }}</div>
            <div class="mt-1 font-semibold text-text">${{ userBalance.toFixed(2) }}</div>
          </div>
          <div class="rounded-lg bg-muted p-3 text-sm">
            <div class="text-text-muted">{{ t('payment.admin.orderAmount') }}</div>
            <div class="mt-1 font-semibold text-text">{{ order?.order_type === 'balance' ? '$' : '¥' }}{{ order?.amount?.toFixed(2) }}</div>
          </div>
        </div>
      ...
      <div>
        ...
        <div class="relative">
          <span class="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted">{{ order?.order_type === 'balance' ? '$' : '¥' }}</span>
          <input
