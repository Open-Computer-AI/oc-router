<template>
  ...
  >
    ...
    <div v-if="loading" class="py-6 text-center text-sm text-text-dim">
      {{ t('common.loading') }}
    </div>

    <div v-else-if="monitors.length === 0" class="py-6 text-center text-sm text-text-dim">
    ...
    <div v-else>
      ...
      <ul class="max-h-80 divide-y divide-border overflow-y-auto rounded-lg border border-border">
        ...
        >
          ...
          <span class="text-xs text-text-dim">{{ m.provider }}</span>
          <span v-if="m.provider === 'openai'" class="text-xs text-text-dim">{{ m.api_mode }}</span>
          <span
            v-if="!m.enabled"
            class="ml-auto rounded bg-muted px-1.5 py-0.5 text-xs text-text-muted"
          >
