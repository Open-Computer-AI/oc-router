  ...
  >
    <div class="space-y-3">
      ...
      <div class="relative">
        ...
        />
        <svg class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-text-dim" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
        </svg>
      </div>

      <div v-if="loading" class="py-6 text-center text-sm text-text-muted">
        {{ t('common.loading') }}
      </div>
      <div v-else-if="filteredKeys.length === 0" class="py-6 text-center text-sm text-text-muted">
      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-border">
        <table class="w-full text-sm">
          <thead class="bg-muted sticky top-0 z-10">
            <tr class="text-left text-xs font-medium uppercase tracking-wider text-text-muted">
          ...
          <tbody class="divide-y divide-border">
            <tr
              ...
              @click="$emit('pick', k)"
            >
              <td class="px-3 py-2 font-medium text-text">{{ k.name }}</td>
              <td class="px-3 py-2 font-mono text-xs text-text-muted">{{ maskApiKey(k.key) }}</td>
              <td class="px-3 py-2">
                <GroupBadge
                  v-if="k.group"
                  :name="k.group.name"
