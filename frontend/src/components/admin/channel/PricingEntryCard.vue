  ...
  <div class="rounded-lg border border-border bg-muted p-3">
    ...
    >
      <Icon
        ...
        size="sm"
        :stroke-width="2"
        class="flex-shrink-0 text-text-dim transition-transform duration-200"
      />
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1">
          ...
          <span
            ...
            class="whitespace-nowrap text-xs text-text-dim"
          >
        ...
        <!-- Billing mode badge -->
        <span
          class="flex-shrink-0 rounded-full bg-accent-muted px-2 py-0.5 text-xs font-medium text-accent"
        >
          {{ billingModeLabel }}
        </span>
      </div>
      ...
      <div v-else class="flex-1 text-xs font-medium text-text-muted">
      ...
      <button
        ...
        class="flex-shrink-0 rounded p-1 text-text-dim hover:text-red-500"
      >
      ...
      <div class="collapsible-inner">
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-text-muted">
            ...
            <span class="ml-1 font-normal text-text-dim">$/MTok</span>
          </label>
          <div class="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-5">
            <div>
              <label class="text-xs text-text-dim">{{ t('admin.channels.form.inputPrice', '输入') }}</label>
            ...
            <div>
              <label class="text-xs text-text-dim">{{ t('admin.channels.form.outputPrice', '输出') }}</label>
              <input :value="entry.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder', '默认')" />
            </div>
            <div>
              <label class="text-xs text-text-dim">{{ t('admin.channels.form.cacheWritePrice', '缓存写入') }}</label>
              <input :value="entry.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder', '默认')" />
            </div>
            <div>
              <label class="text-xs text-text-dim">{{ t('admin.channels.form.cacheReadPrice', '缓存读取') }}</label>
              <input :value="entry.cache_read_price" @input="emitField('cache_read_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder', '默认')" />
            </div>
            <div>
              <label class="text-xs text-text-dim">{{ t('admin.channels.form.imageTokenPrice', '图片输出') }}</label>
          ...
          <div class="mt-3">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-text-muted">
                ...
                <span class="ml-1 font-normal text-text-dim">(min, max]</span>
              </label>
              <button type="button" @click="addInterval" class="text-xs text-accent hover:text-accent">
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <label class="mt-3 block text-xs font-medium text-text-muted">
            ...
            <span class="ml-1 font-normal text-text-dim">$</span>
          </label>
          <div class="mt-1 w-48">
          ...
          <div v-else class="mt-2 rounded border border-dashed border-border p-3 text-center text-xs text-text-dim">
        ...
        <div v-else-if="entry.billing_mode === 'image'">
          ...
          <div class="mt-3 flex items-center justify-between">
            ...
            <button type="button" @click="addImageTier" class="text-xs text-accent hover:text-accent">
              + {{ t('admin.channels.form.addTier', 'Add层级') }}
            </button>
          </div>
          <div v-if="entry.intervals && entry.intervals.length > 0" class="mt-2 space-y-2">
