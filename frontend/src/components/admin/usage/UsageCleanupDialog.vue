    ...
    <div class="space-y-4">
      ...
      <div class="rounded-xl border border-border p-4">
        ...
        <div class="mt-3 space-y-2">
          <div v-if="tasksLoading" class="text-sm text-text-muted">
            {{ t('admin.usage.cleanup.loadingTasks') }}
          </div>
          <div v-else-if="tasks.length === 0" class="text-sm text-text-muted">
          ...
          <div v-else class="space-y-2">
            <div
              v-for="task in tasks"
              :key="task.id"
              class="flex flex-col gap-2 rounded-lg border border-border px-3 py-2 text-sm text-text"
            >
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-text-dim">#{{ task.id }}</span>
                  <button
                    v-if="canCancel(task)"
                    type="button"
