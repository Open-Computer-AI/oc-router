  ...
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      ...
      <div class="flex items-center gap-4 rounded-2xl bg-gradient-to-r from-primary-50 to-primary-100 p-5 dark:from-primary-900/30 dark:to-primary-800/20">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-bg-elevated shadow-sm">
          <span class="text-2xl font-semibold text-accent">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="flex-1">
          <p class="text-lg font-semibold text-text">{{ user.email }}</p>
          <p class="mt-1 text-sm text-text-muted">{{ t('admin.users.groupConfigHint', { email: user.email }) }}</p>
        </div>
      </div>

      <!-- 加载Status -->
      <div v-if="loading" class="flex justify-center py-12">
        <svg class="h-10 w-10 animate-spin text-accent" fill="none" viewBox="0 0 24 24">
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-text">{{ t('admin.users.exclusiveGroups') }}</h4>
            <span class="text-xs text-text-dim">({{ exclusiveGroupConfigs.filter(c => c.isSelected).length }}/{{ exclusiveGroupConfigs.length }})</span>
          </div>
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="flex-shrink-0">
                  <label class="relative flex h-6 w-6 cursor-pointer items-center justify-center">
                    <input
                      ...
                      class="peer sr-only"
                    />
                    <div class="h-5 w-5 rounded-md border-2 border-border transition-all peer-checked:border-accent peer-checked:bg-accent">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-base font-semibold text-text">{{ config.groupName }}</span>
                  ...
                  <div class="mt-1.5 flex items-center gap-3 text-sm">
                    ...
                    <span class="text-text-muted">
                      {{ t('admin.users.defaultRate') }}: <span class="font-medium text-text">{{ config.defaultRate }}x</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  <label class="text-sm font-medium text-text-muted">{{ t('admin.users.customRate') }}</label>
                  <input
                    type="number"
                    step="0.001"
                    min="0.001"
                    ...
                    class="hide-spinner w-24 rounded-lg border border-border bg-bg-elevated px-3 py-2 text-sm font-medium transition-colors focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20"
                  />
                </div>
              </div>
            </div>
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <h4 class="text-sm font-semibold text-text">{{ t('admin.users.publicGroups') }}</h4>
            <span class="text-xs text-text-dim">({{ publicGroupConfigs.length }})</span>
          </div>
          <div class="grid gap-3">
            <div
        ...
        <div v-if="groups.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-muted">
            <svg class="h-8 w-8 text-text-dim" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
          </div>
          <p class="text-text-muted">{{ t('common.noGroupsAvailable') }}</p>
        </div>
      </div>
    </div>
