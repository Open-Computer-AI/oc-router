<template>
  ...
  >
    <div v-if="user" class="space-y-4">
      ...
      <div v-if="loading" class="py-10 text-center text-text-muted">{{ t('common.loading') }}</div>
      <div v-else class="overflow-x-auto">
        <table class="min-w-full text-sm">
          ...
          <tbody>
            <tr v-for="row in quotas" :key="row.platform" class="border-b border-border">
              <td class="px-3 py-2 font-mono text-text">{{ row.platform }}</td>
              <td class="px-3 py-2">
                <div class="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    class="text-xs text-text-dim hover:text-amber-500 disabled:opacity-50"
        ...
        <p class="mt-3 text-xs text-text-muted">{{ t('admin.users.platformQuota.hint') }}</p>
        <div class="mt-3">
...
function onClearAll() {
  // 二次Confirm：一键清空All平台的 daily/weekly/monthly 限额属于高风险批量Actions，
  // 误点后所有平台变为"无限额"，且本地无 undo 机制（需要逐个手动重填或CancelSave）。
...
async function onSave() {
  if (!props.user) return
  // 校验所有 input：v-model.number 在User输入"0."等中间Status时会写回 NaN，
  // 之前的 normalizeLimit(NaN) 静默Back null（"无限制"），把"有限额"配置悄悄改成"无限制"。
  // 这里在 save 前显式检测 NaN，提示User修正后再Submit。
...
// 仅在合法输入下Back数字：null/undefined/NaN/±Inf/负数 → null（视为"无限额"）。
// 调用方负责在 NaN 路径上做单独的User提示（见 onSave）。
