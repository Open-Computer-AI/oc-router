    ...
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div class="rounded-lg border border-border bg-muted p-3">
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-text">{{ oldGroup.name }}</span>
          <Icon name="arrowRight" size="sm" class="ml-auto text-text-dim" />
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        ...
        >
          ...
          <div class="flex-1">
            ...
            <span class="ml-2 text-xs text-text-dim">{{ group.platform }}</span>
          </div>
        </label>
      </div>
