<template>
  <div class="rounded-2xl border border-border bg-muted p-4">
    ...
    <div v-if="mode === 'custom'" class="mt-4 space-y-4">
      ...
      <div v-if="anyOf.length === 0" class="rounded-xl border border-dashed border-border p-4 text-sm text-text-muted">
      ...
      <div
        ...
        :key="groupIndex"
        class="rounded-2xl border border-border bg-bg-elevated p-4 shadow-sm"
      >
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="text-sm font-medium text-text">
              {{ t('admin.announcements.form.targetingCustom') }} #{{ groupIndex + 1 }}
              <span class="ml-2 text-xs font-normal text-text-muted">AND ({{ (group.all_of?.length || 0) }}/50)</span>
            </div>
            <div class="mt-1 text-xs text-text-muted">
