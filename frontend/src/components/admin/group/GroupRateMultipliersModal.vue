    ...
    <div v-if="group" class="space-y-4">
      ...
      <div class="rounded-lg border border-border p-3">
        ...
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-border pt-3">
          <span class="text-xs font-medium text-text-muted">{{ t('admin.groups.batchAdjust') }}</span>
          <div class="flex items-center gap-1.5">
            ...
            <input
              ...
              min="0"
              autocomplete="off"
              class="hide-spinner w-20 rounded border border-border bg-bg-elevated px-2 py-1 text-center text-sm transition-colors focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent/20"
              placeholder="0.5"
            />
            <button
              type="button"
              ...
              <table class="w-full text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-border bg-muted">
                    ...
                    <th class="px-3 py-2 text-left text-xs font-medium text-text-muted">{{ t('admin.groups.columns.rateMultiplier') }}</th>
                    <th v-if="showFinalRate" class="px-3 py-2 text-left text-xs font-medium text-accent">{{ t('admin.groups.finalRate') }}</th>
                ...
                <tbody class="divide-y divide-border">
                  ...
                  >
                    ...
                    <td v-if="showFinalRate" class="whitespace-nowrap px-3 py-2 font-medium text-accent">
...
const loadEntries = async () => {
  ...
  try {
    ...
    // 仅显示已Settings rate_multiplier 的条目；rpm_override 在另一个弹窗管理，保留不动
...
// Save：一次性Submit所有数据（只Submit rate_multiplier；rpm_override 由独立弹窗管理）
