...
const statusMap: Record<OrderStatus, { key: string; class: string }> = {
  ...
  EXPIRED: { key: 'payment.status.expired', class: 'bg-muted text-text' },
  CANCELLED: { key: 'payment.status.cancelled', class: 'bg-muted text-text' },
...
const statusClass = computed(() => {
  const entry = statusMap[props.status]
  return entry?.class ?? 'bg-muted text-text'
})
</script>
