    ...
    <template v-if="outcome === 'success'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-lg font-bold text-text">{{ props.orderType === 'subscription' ? t('payment.result.subscriptionSuccess') : t('payment.result.success') }}</p>
          <div v-if="paidOrder" class="w-full rounded-xl bg-muted p-4">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-text-muted">{{ t('payment.orders.orderId') }}</span>
                <span class="font-medium text-text">#{{ paidOrder.id }}</span>
              </div>
              <div v-if="paidOrder.out_trade_no" class="flex justify-between">
                <span class="text-text-muted">{{ t('payment.orders.orderNo') }}</span>
                <span class="font-medium text-text">{{ paidOrder.out_trade_no }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-text-muted">{{ t('payment.orders.amount') }}</span>
                <span class="font-medium text-text">{{ paidOrder.order_type === 'balance' ? '$' + paidOrder.amount.toFixed(2) : formatGatewayAmount(paidOrder.amount) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-text-muted">{{ t('payment.orders.payAmount') }}</span>
                <span class="font-medium text-text">{{ formatGatewayAmount(paidOrder.pay_amount) }}</span>
              </div>
            </div>
          </div>
      ...
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          <div class="flex h-16 w-16 items-center justify-center rounded-full bg-muted">
            <svg class="h-8 w-8 text-text-dim" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </div>
          <p class="text-lg font-bold text-text">{{ t('payment.qr.cancelled') }}</p>
          <p class="text-sm text-text-muted">{{ t('payment.qr.cancelledDesc') }}</p>
          <button class="btn btn-primary" @click="handleDone">{{ t('common.confirm') }}</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="outcome === 'expired'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-lg font-bold text-text">{{ t('payment.qr.expired') }}</p>
          <p class="text-sm text-text-muted">{{ t('payment.qr.expiredDesc') }}</p>
          <button class="btn btn-primary" @click="handleDone">{{ t('common.confirm') }}</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="qrUrl">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4">
          <p class="text-lg font-semibold text-text">{{ scanTitle }}</p>
          ...
          <p v-if="scanHint" class="text-center text-sm text-text-muted">{{ scanHint }}</p>
      ...
      <div class="card p-4 text-center">
        <p class="text-sm text-text-muted">{{ t('payment.qr.expiresIn') }}</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-text">{{ countdownDisplay }}</p>
        <p class="mt-1 text-xs text-text-dim">{{ t('payment.qr.waitingPayment') }}</p>
      </div>
    ...
    <template v-else>
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-sm text-text-muted">{{ t('payment.qr.payInNewWindowHint') }}</p>
          <button v-if="payUrl" class="btn btn-secondary text-sm" @click="reopenPopup">
            {{ t('payment.qr.openPayWindow') }}
          </button>
        </div>
