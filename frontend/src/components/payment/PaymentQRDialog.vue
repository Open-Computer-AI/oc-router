<template>
  <BaseDialog :show="show" :title="dialogTitle" width="narrow" @close="handleClose">
    ...
    <div v-if="!success" class="flex flex-col items-center space-y-4">
      <!-- QR Code mode -->
      <template v-if="qrUrl">
        <div class="rounded-2xl bg-bg-elevated p-4 shadow-sm">
          <canvas ref="qrCanvas" class="mx-auto"></canvas>
        </div>
        <p v-if="scanHint" class="text-center text-sm text-text-muted">
          {{ scanHint }}
        </p>
      </template>
      ...
      <template v-else>
        <div class="flex flex-col items-center py-4">
          ...
          <p class="mt-4 text-sm text-text-muted">{{ t('payment.qr.payInNewWindowHint') }}</p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-text-muted">{{ qrUrl ? t('payment.qr.expiresIn') : '' }}</p>
    ...
    <div v-else class="flex flex-col items-center space-y-4 py-4">
      ...
      <p class="text-lg font-bold text-text">{{ t('payment.result.success') }}</p>
      <div v-if="paidOrder" class="w-full rounded-xl bg-muted p-4">
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-medium text-text">{{ paidOrder.order_type === 'balance' ? '$' : '¥' }}{{ paidOrder.amount.toFixed(2) }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-text-muted">{{ t('payment.orders.payAmount') }}</span>
            <span class="font-medium text-text">¥{{ paidOrder.pay_amount.toFixed(2) }}</span>
          </div>
        </div>
      </div>
    </div>
