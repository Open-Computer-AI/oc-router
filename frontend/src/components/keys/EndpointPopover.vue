...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    <div
      ...
      :key="index"
      class="flex items-center gap-1.5 rounded-lg border border-border bg-bg-elevated px-2.5 py-1.5 text-xs transition-colors hover:border-accent"
    >
      <span class="font-medium text-text-muted">{{ item.name }}</span>
      <span
        v-if="item.isDefault"
        class="rounded bg-accent-muted px-1 py-px text-[10px] font-medium leading-tight text-accent"
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        <div
          class="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max max-w-[24rem] -translate-x-1/2 translate-y-1 rounded-xl border border-border bg-bg-elevated px-3 py-2.5 text-left opacity-0 shadow-[0_14px_36px_-20px_rgba(15,23,42,0.35)] ring-1 ring-border transition-all duration-150 group-hover/endpoint:translate-y-0 group-hover/endpoint:opacity-100 group-focus-within/endpoint:translate-y-0 group-focus-within/endpoint:opacity-100"
        >
          <p
            v-if="item.description"
            class="max-w-[24rem] break-words text-xs leading-5 text-text-muted"
          >
            {{ item.description }}
          </p>
          <p
          ...
          <div class="absolute left-1/2 top-full h-3 w-3 -translate-x-1/2 -translate-y-1/2 rotate-45 border-b border-r border-border bg-bg-elevated"></div>
        </div>

        <code
          class="cursor-pointer font-mono text-text-muted decoration-text-dim decoration-dashed underline-offset-2 hover:text-accent hover:underline focus:text-accent focus:underline focus:outline-none"
          role="button"
          tabindex="0"
        ...
        <a
          ...
          class="rounded p-0.5 text-text-dim transition-colors hover:text-warning"
