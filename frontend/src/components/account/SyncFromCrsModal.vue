<template>
  ...
  >
    ...
    >
      ...
      <div
        class="rounded-lg bg-muted p-3 text-xs text-text-muted"
      >
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      >
        ...
        <div class="max-h-32 overflow-auto text-xs text-text-muted">
          <div
      ...
      <div v-if="previewResult.new_accounts.length">
        ...
        <div
          class="max-h-48 overflow-auto rounded-lg border border-border p-2"
        >
          <label
            v-for="acc in previewResult.new_accounts"
            :key="acc.crs_account_id"
            class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 hover:bg-muted"
          >
            <input
              type="checkbox"
            ...
            >{{ acc.platform }} / {{ acc.type }}</span>
            <span class="truncate text-sm text-text">{{ acc.name }}</span>
          </label>
        </div>
      ...
      <div class="flex items-center gap-2 text-xs text-text-muted">
        ...
        <span :class="form.sync_proxies ? 'text-green-600 dark:text-green-400' : 'text-text-dim'">
      ...
      <div
        ...
        class="rounded-lg bg-muted p-4 text-center text-sm text-text-muted"
      >
    ...
    <div v-else-if="currentStep === 'result' && result" class="space-y-4">
      ...
      >
        ...
        <div v-if="errorItems.length" class="mt-2">
          ...
          <div
            class="mt-2 max-h-48 overflow-auto rounded-lg bg-muted p-3 font-mono text-xs"
          >
