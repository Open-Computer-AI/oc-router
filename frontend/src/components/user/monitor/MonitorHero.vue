<template>
  <section class="py-3 md:py-4">
    <div class="flex items-center justify-end gap-3 flex-wrap">
      <div
        role="tablist"
        class="inline-flex p-0.5 rounded-xl bg-muted border border-border text-xs"
      >
        <button
          ...
          :class="window === opt.value
            ? 'bg-bg-elevated shadow-sm text-text font-semibold'
      ...
      <button
        type="button"
        class="h-8 w-8 rounded-lg flex items-center justify-center text-text-muted hover:text-text hover:bg-muted transition-colors disabled:opacity-50"
        :disabled="loading"
