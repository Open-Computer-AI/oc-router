<template>
  <div class="mt-4 pt-3 border-t border-border">
    <div
      class="flex justify-between text-[10px] font-semibold uppercase tracking-widest text-text-dim mb-2"
    >
    ...
    <div
      v-if="maintenance"
      class="flex h-5 w-full items-center justify-center rounded border border-dashed border-border text-[10px] uppercase tracking-widest text-text-dim"
    >
    ...
    <div
      class="mt-1 flex justify-between text-[9px] uppercase tracking-widest text-text-dim"
    >
...
// 长绿(Normal) > 中黄(降级) > 短红(失败/系统错误) > 很短灰(未测试)。
