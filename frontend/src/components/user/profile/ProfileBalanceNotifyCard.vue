    ...
    <div class="px-6 py-6 space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <label class="relative inline-flex items-center cursor-pointer">
          ...
          <div class="w-11 h-6 bg-muted peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-accent/30 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-border after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-accent"></div>
        </label>
      </div>
      ...
      <template v-if="notifyEnabled">
        ...
        <div>
          <label class="input-label">
            ...
            <span class="text-xs text-text-dim ml-2">{{ t('profile.balanceNotify.thresholdHint') }}</span>
          </label>
        ...
        <div>
          ...
          <p class="mb-2 text-xs text-warning">{{ t('profile.balanceNotify.extraEmailsHint') }}</p>

          <!-- Saved email entries -->
          <div v-if="emailEntries.length > 0" class="space-y-2 mb-3">
            <div v-for="(entry, idx) in emailEntries" :key="idx"
              class="flex items-center justify-between px-3 py-2 bg-muted rounded-lg">
              <div class="flex items-center gap-2 min-w-0 flex-1">
                ...
                <span class="text-sm text-text truncate">{{ entry.email }}</span>
              </div>
                ...
                <template v-if="!entry.verified">
                  ...
                  <template v-if="verifyingEmail === entry.email">
                    <input
                      ...
                      type="text"
                      maxlength="6"
                      class="w-20 rounded border border-border px-2 py-1 text-xs"
                    ...
                    <span v-if="verifyCountdown > 0" class="text-xs text-text-dim">{{ verifyCountdown }}s</span>
                    <button v-else @click="sendCodeForSaved(entry.email)" :disabled="sendingSavedCode" class="text-xs text-text-muted hover:text-text">
                      {{ t('profile.balanceNotify.resend') }}
                    </button>
                    <button @click="verifyingEmail = ''" class="text-xs text-text-dim hover:text-text-muted">
                      {{ t('common.cancel') }}
                    </button>
                  </template>
                  <template v-else>
                    ...
                    <span class="text-xs text-warning">{{ t('profile.balanceNotify.unverified') }}</span>
                  </template>
                </template>
                <span v-else class="text-xs text-success">{{ t('profile.balanceNotify.verified') }}</span>
                <button @click="handleRemoveEmail(entry.email)" class="text-danger hover:text-danger text-xs">
          ...
          <div v-if="pendingEmails.length > 0" class="space-y-2 mb-3">
            <div v-for="(pe, idx) in pendingEmails" :key="pe.email"
              ...
              <span class="flex-1 text-sm text-text">{{ pe.email }}</span>
              <div v-if="!pe.codeSent" class="flex items-center gap-1">
                ...
                <button @click="pendingEmails.splice(idx, 1)" class="text-xs text-danger hover:text-danger ml-1">
                  {{ t('profile.balanceNotify.removeEmail') }}
                </button>
              </div>
              <div v-else class="flex items-center gap-1">
                ...
                <span v-if="pe.countdown > 0" class="text-xs text-text-dim">{{ pe.countdown }}s</span>
                <button v-else @click="sendCodeFor(idx)" :disabled="pe.sending" class="text-xs text-text-muted hover:text-text">
                  {{ t('profile.balanceNotify.resend') }}
                </button>
              </div>
            </div>
