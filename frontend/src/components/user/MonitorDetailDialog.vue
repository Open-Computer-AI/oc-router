<template>
  <BaseDialog
    ...
    @close="$emit('close')"
  >
    <div v-if="loading" class="py-8 text-center text-sm text-text-muted">
      {{ t('common.loading') }}
    </div>
    <div v-else-if="!detail" class="py-8 text-center text-sm text-text-muted">
    ...
    <div v-else class="overflow-x-auto">
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          ...
          >
            <td class="py-2 pr-3 font-medium text-text">{{ m.model }}</td>
            <td class="py-2 pr-3">
              <span
            ...
            <td class="py-2 pr-3 text-text">{{ formatLatency(m.latest_latency_ms) }}</td>
            <td class="py-2 pr-3 text-text">{{ formatPercent(m.availability_7d) }}</td>
            <td class="py-2 pr-3 text-text">{{ formatPercent(m.availability_15d) }}</td>
            <td class="py-2 pr-3 text-text">{{ formatPercent(m.availability_30d) }}</td>
            <td class="py-2 pr-3 text-text">{{ formatLatency(m.avg_latency_7d_ms) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
