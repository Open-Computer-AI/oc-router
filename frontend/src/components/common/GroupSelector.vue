<template>
  <div>
    ...
    <div
      v-if="isSearchable"
      class="flex items-center gap-2 rounded-t-lg border border-b-0 border-border bg-muted px-3 py-2"
    >
    ...
    <div
      :class="[
        ...
        isSearchable
          ? 'rounded-b-lg border border-t-0 border-border bg-muted'
    ...
    >
      <label
        ...
        :key="group.id"
        class="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 transition-colors hover:bg-bg-elevated"
      ...
      >
        <input
          ...
          class="h-3.5 w-3.5 shrink-0 rounded border-border text-accent focus:ring-accent"
        />
        <GroupBadge
          :name="group.name"
      ...
      <div
        ...
        class="col-span-2 py-2 text-center text-sm text-text-muted"
      >
...
const filteredGroups = computed(() => {
  ...
  if (props.platform) {
    // antigravity AccountEnable混合调度后，可选择 anthropic/gemini 分组
