  ...
  <BaseDialog :show="show" :title="t('usage.exporting')" width="narrow" @close="handleCancel">
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between text-sm text-text">
        ...
        <span class="font-medium text-text">{{ normalizedProgress }}%</span>
      </div>
      ...
      <div v-if="estimatedTime" class="text-xs text-text-muted" aria-live="polite" aria-atomic="true">
    ...
    <template #footer>
      <button
        @click="handleCancel"
        type="button"
        class="rounded-md border border-border bg-bg-elevated px-4 py-2 text-sm font-medium text-text hover:bg-muted focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2"
      >
