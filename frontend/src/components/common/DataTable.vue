<template>
  <div v-if="!isDesktopViewport" class="space-y-3">
    <template v-if="loading">
      <div v-for="i in 5" :key="i" class="rounded-lg border border-border bg-bg-elevated p-4">
        <div class="space-y-3">
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="rounded-lg border border-border bg-bg-elevated p-12 text-center">
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-4 h-12 w-12 text-text-dim"
            />
    ...
    <template v-else>
      ...
      >
        <div class="space-y-3">
          ...
          >
            <span class="font-mono text-[11px] uppercase tracking-widest text-text-dim">
              {{ column.label }}
            </span>
            <div class="text-right text-sm text-text">
  ...
  >
    <table class="w-full min-w-max divide-y divide-border">
      <thead class="table-header bg-muted">
        <tr>
          <th
            ...
            scope="col"
            :class="[
              'sticky-header-cell py-3 text-left font-mono text-[11px] uppercase tracking-widest text-text-dim',
          ...
          >
            ...
            >
              <div class="flex items-center space-x-1">
                ...
                <span v-if="column.sortable" class="text-text-dim">
                  <svg
      ...
      <tbody class="table-body divide-y divide-border bg-bg-elevated">
        <!-- Loading skeleton -->
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['py-12 text-center text-text-muted', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          >
            <td
              ...
              :key="column.key"
              :class="[
                'whitespace-nowrap py-4 text-sm text-text',
                getAdaptivePaddingClass(),
