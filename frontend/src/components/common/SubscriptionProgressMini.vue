<template>
  <div v-if="hasActiveSubscriptions" class="relative" ref="containerRef">
    ...
    <transition name="dropdown">
      <div
        v-if="tooltipOpen"
        class="absolute right-0 z-50 mt-2 w-[340px] overflow-hidden rounded-xl border border-border bg-bg-elevated shadow-xl"
      >
        ...
        <div class="max-h-64 overflow-y-auto">
          <div
            ...
            :key="subscription.id"
            class="border-b border-border/50 p-3 last:border-b-0"
          >
            ...
            <div class="space-y-1.5">
              ...
              <template v-else>
                <div v-if="subscription.group?.daily_limit_usd" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-text-muted">{{
                  ...
                  <span class="w-24 flex-shrink-0 text-right text-[10px] text-text-muted">
                    {{
        ...
        <div class="border-t border-border p-2">
          <router-link
            to="/subscriptions"
            @click="closeTooltip"
            class="block w-full py-1 text-center text-xs text-accent hover:underline"
          >
...
function getProgressBarClass(used: number | undefined, limit: number | null | undefined): string {
  if (!limit || limit === 0) return 'bg-text-dim'
