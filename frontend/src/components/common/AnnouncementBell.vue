<template>
  <div>
    ...
    <button
      @click="openModal"
      class="relative flex h-9 w-9 items-center justify-center rounded-lg text-text-muted transition-all hover:bg-muted hover:scale-105"
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        ...
        >
          <div
            class="w-full max-w-[620px] overflow-hidden rounded-3xl bg-bg-elevated shadow-2xl ring-1 ring-black/5 dark:ring-white/10"
            @click.stop
          >
            <!-- Header with Gradient -->
            <div class="relative overflow-hidden border-b border-border bg-gradient-to-br from-blue-50/50 to-indigo-50/30 px-6 py-5 dark:from-blue-900/10 dark:to-indigo-900/5">
              <div class="relative z-10 flex items-start justify-between">
                <div>
                  ...
                  <p v-if="unreadCount > 0" class="mt-2 text-sm text-text-muted">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <button
                    @click="closeModal"
                    class="flex h-9 w-9 items-center justify-center rounded-lg bg-bg-elevated/50 text-text-muted backdrop-blur-sm transition-all hover:bg-bg-elevated hover:text-text"
            ...
            <div class="max-h-[65vh] overflow-y-auto">
              ...
              <div v-if="loading" class="flex items-center justify-center py-16">
                <div class="relative">
                  <div class="h-12 w-12 animate-spin rounded-full border-4 border-muted border-t-accent"></div>
              ...
              <div v-else-if="announcements.length > 0">
                <div
                  ...
                  :key="item.id"
                  class="group relative flex items-center gap-4 border-b border-border px-6 py-4 transition-all hover:bg-muted"
                ...
                >
                  ...
                  <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center">
                    ...
                    <div
                      v-else
                      class="flex h-10 w-10 items-center justify-center rounded-xl bg-muted text-text-dim"
                    >
                  ...
                  <div class="flex min-w-0 flex-1 items-center justify-between gap-4">
                    ...
                    <div class="flex-shrink-0">
                      <svg
                        class="h-5 w-5 text-text-dim transition-transform group-hover:translate-x-1"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        stroke-width="2"
              ...
              <div v-else class="flex flex-col items-center justify-center py-16">
                <div class="relative mb-4">
                  <div class="flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-muted to-muted">
                    <Icon name="inbox" size="xl" class="text-text-dim" />
                  </div>
                ...
                <p class="text-sm font-medium text-text">{{ t('announcements.empty') }}</p>
                <p class="mt-1 text-xs text-text-muted">{{ t('announcements.emptyDescription') }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        ...
        >
          <div
            class="w-full max-w-[780px] overflow-hidden rounded-3xl bg-bg-elevated shadow-2xl ring-1 ring-black/5 dark:ring-white/10"
            @click.stop
          >
            <!-- Header with Decorative Elements -->
            <div class="relative overflow-hidden border-b border-border bg-gradient-to-br from-blue-50/80 via-indigo-50/50 to-purple-50/30 px-8 py-6 dark:from-blue-900/20 dark:via-indigo-900/10 dark:to-purple-900/5">
              ...
              <div class="relative z-10 flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  ...
                  <!-- Title -->
                  <h2 class="mb-3 text-2xl font-bold leading-tight text-text">
                ...
                <button
                  @click="closeDetail"
                  class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-bg-elevated/50 text-text-muted backdrop-blur-sm transition-all hover:bg-bg-elevated hover:text-text hover:shadow-lg"
                  :aria-label="t('common.close')"
                >
                  <Icon name="x" size="md" />
                </button>
            ...
            <div class="max-h-[60vh] overflow-y-auto bg-bg-elevated px-8 py-8">
            ...
            <div class="border-t border-border bg-muted/50 px-8 py-5">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                  <button
                    @click="closeDetail"
                    class="rounded-xl border border-border bg-bg-elevated px-5 py-2.5 text-sm font-medium text-text shadow-sm transition-all hover:bg-muted hover:shadow"
                  >
                    {{ t('common.close') }}
                  </button>
                  <button
...
.markdown-body h1 {
  @apply mb-6 mt-8 border-b border-border pb-3 text-3xl font-bold text-text;
}

.markdown-body h2 {
  @apply mb-4 mt-7 border-b border-border pb-2 text-2xl font-bold text-text;
}
...
.markdown-body h4 {
  @apply mb-2 mt-5 text-lg font-semibold text-text;
}
...
.markdown-body blockquote {
  @apply relative my-5 border-l-4 border-blue-500 bg-blue-50/50 py-3 pl-5 pr-4 italic text-text dark:border-blue-400 dark:bg-blue-900/10;
}
...
.markdown-body code {
  @apply rounded-lg bg-muted px-2 py-1 text-[13px] font-mono text-pink-600 dark:text-pink-400;
}

.markdown-body pre {
  @apply my-5 overflow-x-auto rounded-xl border border-border bg-muted p-5;
}

.markdown-body pre code {
  @apply bg-transparent p-0 text-[13px] text-text;
}
...
.markdown-body table {
  @apply mb-5 w-full overflow-hidden rounded-lg border border-border;
}
...
.markdown-body th {
  @apply bg-gradient-to-br from-blue-50 to-indigo-50 font-semibold text-text dark:from-blue-900/20 dark:to-indigo-900/10;
}
...
.markdown-body img {
  @apply my-5 max-w-full rounded-xl border border-border shadow-md;
}
