<template>
  <div
    class="flex items-center justify-between border-t border-border bg-bg-elevated px-4 py-3 sm:px-6"
  >
    <div class="flex flex-1 items-center justify-between sm:hidden">
      ...
      <span class="font-mono text-xs text-text-dim uppercase tracking-wider">
    ...
    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
      ...
      <div class="flex items-center space-x-4">
        <p class="font-mono text-[11px] text-text-dim uppercase tracking-wider">
          {{ t('pagination.showing') }}
          <span class="font-semibold text-text">{{ fromItem }}</span>
          {{ t('pagination.to') }}
          <span class="font-semibold text-text">{{ toItem }}</span>
          {{ t('pagination.of') }}
        ...
        <div v-if="showJump" class="flex items-center space-x-2">
          <span class="text-xs text-text-muted">{{ t('pagination.jumpTo') }}</span>
          <input
            v-model="jumpPage"
            type="number"
            min="1"
      ...
      <nav
        class="relative z-0 inline-flex items-center gap-1"
      ...
      >
        ...
        <button
          @click="goToPage(page - 1)"
          :disabled="page === 1"
          class="btn btn-ghost btn-icon btn-sm disabled:opacity-50"
