<template>
  <aside
    class="sidebar fixed inset-y-2 left-2 z-40 flex flex-col overflow-hidden"
    :class="[
      sidebarCollapsed ? 'w-[60px]' : 'w-[272px]',
      { '-translate-x-[calc(100%+16px)] lg:translate-x-0': !mobileOpen }
    ]"
  >
    <!-- Header: Logo + Toggle -->
    <div class="sidebar-header flex-shrink-0" :class="{ 'justify-center px-2': sidebarCollapsed }">
      <!-- Expanded: Wordmark -->
      <template v-if="!sidebarCollapsed">
        <div class="flex items-center min-w-0 flex-1">
          <img v-if="settingsLoaded" :src="siteLogo || '/oc-router-logo.png'" alt="OC Router" class="h-10 w-auto flex-shrink-0 object-contain dark:invert dark:brightness-200" />
        </div>
        <button
          @click="toggleSidebar"
          class="flex-shrink-0 rounded-lg p-1.5 text-text-dim transition-colors hover:bg-muted hover:text-text"
      ...
      <template v-else>
        <button
          @click="toggleSidebar"
          class="flex h-8 w-10 items-center justify-center overflow-hidden rounded-lg transition-colors hover:bg-muted"
          :title="t('nav.expand')"
        >
          <img v-if="settingsLoaded" :src="siteLogo || '/oc-router-logo.png'" alt="OC Router" class="h-5 w-auto object-contain object-left dark:invert dark:brightness-200" style="clip-path: inset(0 60% 0 0);" />
        </button>
      </template>
    </div>

    <!-- New API Key button (user view) -->
    <div v-if="!isAdmin && !sidebarCollapsed && !appStore.backendModeEnabled" class="px-2 pt-2">
      <router-link
        to="/keys"
    ...
    <!-- Navigation -->
    <nav class="sidebar-nav flex-1 scrollbar-hide">
      ...
      <template v-if="isAdmin">
        ...
        <div class="sidebar-section">
          <template v-for="item in adminNavItems" :key="item.path">
            ...
            <template v-if="item.children?.length">
              ...
              <!-- Children -->
              <div v-if="!sidebarCollapsed && isGroupExpanded(item)" class="mb-1 ml-6 border-l border-border pl-2">
                <router-link
                  v-for="child in item.children"
                  :key="child.path"
                  :to="child.path"
                  class="sidebar-link mb-0.5 py-1.5 text-[13px]"
        ...
        <!-- My Account section for Admin (hidden in simple mode) -->
        <div v-if="!authStore.isSimpleMode" class="sidebar-section">
          <div class="sidebar-section-title" :class="{ 'opacity-0': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
            {{ t('nav.myAccount') }}
          </div>
    ...
    <div class="flex-shrink-0 border-t border-border p-2">
      ...
      <button
        ...
        :class="{ 'sidebar-link-collapsed justify-center': sidebarCollapsed }"
        :title="sidebarCollapsed ? (isDark ? t('nav.lightMode') : t('nav.darkMode')) : undefined"
      >
        <SunIcon v-if="isDark" class="h-[18px] w-[18px] flex-shrink-0 text-warning" />
      ...
      <div
        ...
        class="group relative flex items-center gap-2.5 rounded-lg px-3 py-2 transition-colors hover:bg-muted"
      >
        <!-- Avatar -->
        <div class="flex h-8 w-8 flex-shrink-0 items-center justify-center overflow-hidden rounded-full bg-accent text-xs font-medium text-accent-fg">
          <img
            v-if="avatarUrl"
            :src="avatarUrl"
            :alt="displayName"
        ...
        <button
          @click="handleLogout"
          class="absolute right-2 top-1/2 -translate-y-1/2 rounded-md p-1 text-text-dim opacity-0 transition-all hover:bg-danger-muted hover:text-danger group-hover:opacity-100"
          :title="t('nav.logout')"
        >
      ...
      <button
        v-if="user && sidebarCollapsed"
        @click="handleLogout"
        class="flex w-full items-center justify-center rounded-lg py-2 transition-colors hover:bg-muted"
        :title="displayName + ' — ' + t('nav.logout')"
      >
        <div class="flex h-7 w-7 items-center justify-center overflow-hidden rounded-full bg-accent text-[10px] font-medium text-accent-fg">
          <img
            v-if="avatarUrl"
            :src="avatarUrl"
            :alt="displayName"
...
const ChannelIcon = {
  render: () =>
    h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' }, [
      h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M6.429 9.75L2.25 12l4.179 2.25m0-4.5l5.571 3 5.571-3m-11.142 0L2.25 7.5 12 2.25l9.75 5.25-4.179 2.25m0 0l4.179 2.25L12 17.25 2.25 12m15.321-2.25l4.179 2.25L12 17.25l-9.75-5.25' })
    ])
}
...
const RechargeSubscriptionIcon = {
  render: () =>
    h('svg', { fill: 'currentColor', viewBox: '0 0 1024 1024' }, [
      h('path', { d: 'M512 992C247.3 992 32 776.7 32 512S247.3 32 512 32s480 215.3 480 480c0 84.4-22.2 167.4-64.2 240-8.9 15.3-28.4 20.6-43.7 11.7-15.3-8.8-20.5-28.4-11.7-43.7 36.4-62.9 55.6-134.8 55.6-208 0-229.4-186.6-416-416-416S96 282.6 96 512s186.6 416 416 416c17.7 0 32 14.3 32 32s-14.3 32-32 32z' }),
      h('path', { d: 'M640 512H384c-17.7 0-32-14.3-32-32s14.3-32 32-32h256c17.7 0 32 14.3 32 32s-14.3 32-32 32zM640 640H384c-17.7 0-32-14.3-32-32s14.3-32 32-32h256c17.7 0 32 14.3 32 32s-14.3 32-32 32z' }),
      h('path', { d: 'M512 480c-8.2 0-16.4-3.1-22.6-9.4l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l128 128c12.5 12.5 12.5 32.8 0 45.3-6.3 6.3-14.5 9.4-22.7 9.4z' }),
      h('path', { d: 'M512 480c-8.2 0-16.4-3.1-22.6-9.4-12.5-12.5-12.5-32.8 0-45.3l128-128c12.5-12.5 32.8-12.5 45.3 0s12.5 32.8 0 45.3l-128 128c-6.3 6.3-14.5 9.4-22.7 9.4z' }),
      h('path', { d: 'M512 736c-17.7 0-32-14.3-32-32V448c0-17.7 14.3-32 32-32s32 14.3 32 32v256c0 17.7-14.3 32-32 32zM896 992H512c-17.7 0-32-14.3-32-32s14.3-32 32-32h306.8l-73.4-73.4c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0l128 128c9.2 9.2 11.9 22.9 6.9 34.9S908.9 992 896 992z' })
    ])
}
...
const BellIcon = {
  render: () =>
    h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' }, [
      h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75V9a6 6 0 10-12 0v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0' })
    ])
}
